...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
	<!-- ================================================================== -->
	<!-- TravelFusion_LowFarePlusRS.xsl 												-->
	<!-- ================================================================== -->
				...
				<xsl:otherwise>
					...
					<PricedItineraries>
						<xsl:variable name="priceditin">
							<xsl:choose>
								<xsl:when test="CheckRouting/RouterList/Router/GroupList/Group/ReturnList/Return">
									<xsl:apply-templates select="CheckRouting/RouterList/Router/GroupList" mode="circle"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:apply-templates select="CheckRouting/RouterList/Router/GroupList" mode="oneway"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						<xsl:apply-templates select="msxsl:node-set($priceditin)/PricedItinerary" mode="final">
		...
		<xsl:for-each select="Group/OutwardList/Outward">
			...
			<xsl:for-each select="../../ReturnList/Return">
				<PricedItinerary>
					...
					<AirItinerary>
						<xsl:attribute name="DirectionInd"><xsl:value-of select="'Circle'"/></xsl:attribute>
							...
							<xsl:if test="$fee!='0'">
								<Fees>
	  								<Fee>
	  									...
	  									<xsl:attribute name="FeeType"><xsl:value-of select="'Admin fee'"/></xsl:attribute>
	  									<xsl:attribute name="Amount">
	...
	<xsl:template match="GroupList" mode="oneway">
		<xsl:for-each select="Group/OutwardList/Outward">
				<PricedItinerary>
					...
					<AirItinerary>
						<xsl:attribute name="DirectionInd"><xsl:value-of select="'OneWay'"/></xsl:attribute>
						<OriginDestinationOptions>
							<xsl:apply-templates select="SegmentList"/>
						</OriginDestinationOptions>
					</AirItinerary>
					...
					<Notes><xsl:value-of select="'RoutingId'"/></Notes>
					<Notes><xsl:value-of select="concat('OutwardId:',Id)"/></Notes>
					<TicketingInfo>
						<xsl:variable name="depday">
							<xsl:value-of select="substring(SegmentList/Segment/DepartDate,1,2)" />
						</xsl:variable>
						<xsl:variable name="depmonth">
							<xsl:value-of select="substring(SegmentList/Segment/DepartDate,4,2)" />
						</xsl:variable>
						<xsl:variable name="depyear">
							<xsl:value-of select="substring(SegmentList/Segment/DepartDate,7,4)" />
						</xsl:variable>
						<xsl:attribute name="TicketTimeLimit">
							<xsl:value-of select="$depyear" />-<xsl:value-of select="$depmonth" />-<xsl:value-of select="$depday" />T<xsl:value-of select="substring-after(SegmentList/Segment/DepartDate,'-')" />
							<xsl:text>:00</xsl:text>
						</xsl:attribute>
					</TicketingInfo>
				</PricedItinerary>
