...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	<!-- 
  ================================================================== 
	AmadeusWS_LowFare2RS.xsl 													
	================================================================== 
  Date: 22 Oct 2020 - Kobelev - Removed CrLf from Departure and Arrival flight attributes			
	Date: 17 Aug 2012 - Rastko - added specific decoding for Kulula airline			
	Date: 21 May 2011 - Rastko - corrected penalty amount estimation				
	Date: 16 Aug 2011 - Rastko - corrected slice and dice mapping				
	Date: 18 Apr 2011 - Rastko - added extra test for mapping WEB fares	
	Date: 20 Apr 2010 - Rastko - filter out code share on KL and DL			
	Date: 27 Apr 2010 - Sajith - corrected ticketing time limit mapping	
  ...
  -->
	...
	<xsl:template match="/">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:apply-templates select="Fare_MasterPricerExpertSearchReply" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="Fare_MasterPricerExpertSearchReply">
		<OTA_AirLowFareSearchRS>
	...
	<xsl:template match="recommendation">
		...
		<xsl:choose>
			<xsl:when test="substring($fm,1,1)='1'"></xsl:when>
			<xsl:when test="substring($cs,1,1)='1'"></xsl:when>
			<xsl:otherwise>
				<PricedItinerary>
					...
					<xsl:if test="segmentFlightRef/OD/flightDetails/flightInformation/addProductDetail/electronicTicketing != '' or paxFareProduct/fare/pricingMessage/freeTextQualification/textSubjectQualifier = 'LTD'">
						<TicketingInfo>
							<xsl:if test="paxFareProduct/fare/pricingMessage/freeTextQualification/textSubjectQualifier = 'LTD'">
								<xsl:choose>
									<xsl:when test="count(paxFareProduct/fare/pricingMessage[freeTextQualification/textSubjectQualifier = 'LTD']/description)!='1'">
									...
									<xsl:otherwise>
										<xsl:attribute name="TicketTimeLimit">
											<xsl:text>20</xsl:text>
											<xsl:value-of select="substring(paxFareProduct/fare/pricingMessage[freeTextQualification/textSubjectQualifier = 'LTD']/description[1],18,2)"/>
											<xsl:text>-</xsl:text>
											<xsl:call-template name="month">
												<xsl:with-param name="month">
													<xsl:value-of select="substring(paxFareProduct/fare/pricingMessage[freeTextQualification/textSubjectQualifier = 	'LTD']/description[1],15,3)" />
												</xsl:with-param>
											</xsl:call-template>
											<xsl:text>-</xsl:text>
											<xsl:value-of select="substring(paxFareProduct/fare/pricingMessage[freeTextQualification/textSubjectQualifier = 'LTD']/description[1],13,2)"/>
											<xsl:text>T23:59:00</xsl:text>
										</xsl:attribute>
									</xsl:otherwise>
								</xsl:choose>
	...
	<xsl:template match="paxFareProduct">
		<PTC_FareBreakdown>
			...
			<xsl:choose>
				...
				<xsl:when test="fareDetails/groupOfFares/productInformation/fareProductDetail/fareType='RN'">
					<xsl:attribute name="PricingSource">Private</xsl:attribute>
				</xsl:when>
	...
	<xsl:template match="flightDetails">
        ...
        <xsl:value-of select="concat('20',$depyear, '-',$depmonth,'-',$depday,'T',substring(string(flightInformation/productDateTime/timeOfDeparture),1,2),':',substring(string(flightInformation/productDateTime/timeOfDeparture),3,2),':00')" />
      </xsl:attribute>
      
      <xsl:attribute name="ArrivalDateTime">
				<xsl:value-of select="concat('20',$arryear, '-',$arrmonth,'-',$arrday,'T',substring(string(flightInformation/productDateTime/timeOfArrival),1,2),':',substring(string(flightInformation/productDateTime/timeOfArrival),3,2),':00')" />
      </xsl:attribute>
        
      <!--
      20<xsl:value-of select="$depyear" />-<xsl:value-of select="$depmonth" />-<xsl:value-of select="$depday" />T<xsl:value-of select="substring(string(flightInformation/productDateTime/timeOfDeparture),1,2)" />:<xsl:value-of select="substring(string(flightInformation/productDateTime/timeOfDeparture),3,2)" />:00</xsl:attribute>
	...
	<xsl:template match="pricingMessage" mode="Penalty">
		<xsl:choose>
			<xsl:when test="freeTextQualification/informationType = '70' or freeTextQualification/informationType = '71' or freeTextQualification/informationType = '73' or (freeTextQualification/informationType = '72' and not(../monetaryInformation/monetaryDetail/amount))">
				<ChargesRules>
					<VoluntaryChanges>
						<Penalty>									 
