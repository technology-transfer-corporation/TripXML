...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <!-- Date: 28 Feb 2011 - Rastko - corrected parsing of commission				       -->
  <!-- Date: 16 Feb 2011 - Rastko - added parsing of operating airline				       -->
  <!-- Date: 05 Feb 2011 - Rastko - fixed parsing of CC info in hotel and car segment		       -->
  ...
  <xsl:template match="PNR_RetrieveByRecLocReply | PNR_Reply">
    <OTA_TravelItineraryRS>
      ...
      <xsl:choose>
        ...
        <xsl:when test="generalErrorInfo">
          <Errors>
            <xsl:apply-templates select="generalErrorInfo/messageErrorText/text" mode="error"/>
          </Errors>
        </xsl:when>
      </xsl:choose>
    </OTA_TravelItineraryRS>
            ...
            <Reservation>
              ...
              <GuestCounts>
                <GuestCount>
                  ...
                  <xsl:attribute name="RPH">
                    <xsl:variable name="paxref">
                      <xsl:value-of select="referenceForSegment/reference/number"/>
                    </xsl:variable>
                          ...
                          <xsl:when test="hotelReservationInfo/guaranteeOrDeposit/paymentInfo/paymentDetails/formOfPaymentCode='1'">
                            <PaymentCard>
                              ...
                              <xsl:attribute name="CardCode">
                                <xsl:choose>
                                  <xsl:when test="hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/vendorCode = 'CA'">MC</xsl:when>
                                  <xsl:when test="hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/vendorCode = 'DC'">DN</xsl:when>
                                  <xsl:otherwise>
                                    <xsl:value-of select="hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/vendorCode"/>
                                  </xsl:otherwise>
                                </xsl:choose>
                              </xsl:attribute>
                    ...
                    <GuaranteesAccepted>
                      <GuaranteeAccepted>
                        <PaymentCard>
                          ...
                          <xsl:attribute name="CardCode">
                            <xsl:choose>
                              <xsl:when test="substring(generalOption[optionDetail/type='G']/optionDetail/freetext,3,2) = 'CA'">MC</xsl:when>
                              <xsl:when test="substring(generalOption[optionDetail/type='G']/optionDetail/freetext,3,2) = 'DC'">DN</xsl:when>
                              <xsl:otherwise>
                                <xsl:value-of select="substring(generalOption[optionDetail/type='G']/optionDetail/freetext,3,2)"/>
                              </xsl:otherwise>
                            </xsl:choose>
                          </xsl:attribute>
                          <xsl:attribute name="CardNumber">
  ...
  <xsl:template match="dataElementsIndiv" mode="Payment">
    <FormOfPayment>
      <xsl:choose>
        <xsl:when test="substring(otherDataFreetext/longFreetext,1,2) ='CC' or substring(otherDataFreetext/longFreetext,1,6) = 'PAX CC' or substring(otherDataFreetext/longFreetext,1,6) = 'INF CC'">
          <xsl:attribute name="RPH">
            <xsl:value-of select="position()"/>
          </xsl:attribute>
          <xsl:variable name="cc">
            <xsl:choose>
              <xsl:when test="substring(otherDataFreetext/longFreetext,8,1)='/'">
                <xsl:value-of select="concat(substring(otherDataFreetext/longFreetext,1,5),'C')"/>
                <xsl:value-of select="substring(otherDataFreetext/longFreetext,6,2)"/>
                <xsl:value-of select="substring(otherDataFreetext/longFreetext,9)"/>
              </xsl:when>
              <xsl:when test="substring(otherDataFreetext/longFreetext,4,1)='/'">
                <xsl:value-of select="concat(substring(otherDataFreetext/longFreetext,1,1),'C')"/>
                <xsl:value-of select="substring(otherDataFreetext/longFreetext,2,2)"/>
          ...
          <PaymentCard>
            <xsl:attribute name="CardCode">
              <xsl:choose>
                <xsl:when test="substring(substring-after($cc,'CC'),1,2) = 'CA'">MC</xsl:when>
                <xsl:when test="substring(substring-after($cc,'CC'),1,2) = 'DC'">DN</xsl:when>
                <xsl:otherwise>
          ...
          <xsl:if test="$amt!='' and number($amt)">
            <PaymentAmount>
              <xsl:attribute name="Amount">
                <xsl:value-of select="translate(substring($amt,4),'.','')"/>
              </xsl:attribute>
    ...
    <AgencyCommission>
      <xsl:choose>
        ...
        <xsl:when test="contains(otherDataFreetext/longFreetext,'*AM*')">
          <xsl:attribute name="Amount">
            <xsl:value-of select="substring-after(otherDataFreetext/longFreetext,'*AM*')"/>
          </xsl:attribute>
        </xsl:when>
