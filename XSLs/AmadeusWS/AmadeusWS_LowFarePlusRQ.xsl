...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ttVB="urn:ttVB" exclude-result-prefixes="msxsl ttVB" version="1.0">
	...
	<!-- ================================================================== -->
	<!-- Date: 18 Dec 2012 Rastko - added support for 2 one ways option (MKT)			-->
	<!-- Date: 26 Nov 2012 Rastko - removed near date business logic					-->
	<!-- Date: 29 Oct 2012 Rastko - add ADC for skybird pccs							-->
	<!-- Date: 02 Oct 2012 - Rastko - added XLC option for OneTwoTrip				 -->
	...
	<!-- Date: 28 Oct 2011 Rastko - added specific code for Pacific Islands				-->
	<!-- Date: 27 Sep 2011 Rastko - change pax type JCB into ADT to Amadeus			-->
	<!-- Date: 27 Jun 2011 Rastko - added code for VC and change near dates logic			-->
	<!-- Date: 03 Jun 2011 Rastko - add ADC for skybird pcc							-->
	...
	<!-- Date: 19 Oct 2010 - Rastko - changed MaxResponses to 100 limit				-->
	...
	<!-- Date: 27 Jul 2010 - Rastko - limit max responses to 50							-->
	<!-- Date: 21 Jun 2009 - Rastko														-->
	<!-- ================================================================== -->
	<xsl:output method="xml" omit-xml-declaration="yes" />
	<xsl:variable name="pcc" select="OTA_AirLowFareSearchPlusRQ/POS/Source/@PseudoCityCode"/>
	<xsl:variable name="requestor" select="OTA_AirLowFareSearchPlusRQ/POS/Source/RequestorID/@ID"/>
	<xsl:template match="/">
					...
					<numberOfUnits>
						<xsl:choose>
							<xsl:when test="@MaxResponses != ''">
								<xsl:choose>
									<xsl:when test="@MaxResponses &gt; 100">
										<xsl:value-of select="@MaxResponses"/>
									</xsl:when>
									<xsl:otherwise><xsl:value-of select="@MaxResponses"/></xsl:otherwise>
								</xsl:choose>
							</xsl:when>
							<xsl:when test="TravelPreferences/VendorPref or OriginDestinationInformation/ConnectionLocations or TravelPreferences/FlightTypePref">100</xsl:when>
							<xsl:when test="not(TravelPreferences/VendorPref) and not(OriginDestinationInformation/ConnectionLocations) and not(TravelPreferences/FlightTypePref)">100</xsl:when>
			...
			<fareOptions>
				<pricingTickInfo>
					<pricingTicketing>
						...
						<xsl:if test="@TwoOneWays='true' and count(OriginDestinationInformation)=2">
							<priceType>MTK</priceType>
						</xsl:if>
						<xsl:if test="$requestor='OneTwoTrip'">
						...
						<xsl:choose>
							...
							<xsl:when test="TravelerInfoSummary/PriceRequestInformation/@PricingSource='Both'">
								<priceType>RP</priceType>
								<priceType>RU</priceType>
								<xsl:if test="$pcc='NYC1S219Z' or $pcc=' DTW1S2107' or $pcc='DTW1S214M'">
						...
						<cabinId>
							...
							<cabin>
								<xsl:choose>
									...
									<xsl:when test="TravelPreferences/CabinPref/@Cabin = 'Economy' and TravelPreferences/CabinPref/@PreferLevel = 'Only'">M</xsl:when>
	...
	<xsl:template name="create_type">
		...
		<xsl:if test="$count !=0">
			<traveller>
				<xsl:choose>
					<xsl:when test="@Code='INF' or @Code='IN' or @Code='ITF'">
