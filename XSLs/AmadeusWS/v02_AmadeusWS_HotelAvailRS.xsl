<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ttVB="urn:ttVB" exclude-result-prefixes="msxsl ttVB" version="1.0">
  <!-- ================================================================== -->
  <!-- v02_AmadeusWS_HotelAvailRS.xsl 											       -->
  <!-- ================================================================== -->
  <!-- Date: 02 Jul 2014 - Rastko - issue error wehn no RoomStays returned			-->
  <!-- Date: 19 Jun 2014 - Rastko - corrected total rate mapping						-->
  <!-- Date: 16 Jun 2014 - Rastko - mapped pricing response							-->
  <!-- Date: 14 Mar 2014 - Rastko - new file												-->
  <!-- ================================================================== -->
  <xsl:output method="xml" omit-xml-declaration="yes"/>
  <xsl:variable name="cur" select="OTA_HotelAvailRS/CurrencyConversions/CurrencyConversion/@RequestedCurrencyCode"/>
  <xsl:variable name="currate" select="OTA_HotelAvailRS/CurrencyConversions/CurrencyConversion/@RateConversion"/>
  <xsl:template match="/">
  ...
  <xsl:template match="OTA_HotelAvailRS">
    <OTA_HotelAvailRS>
      ...
      <xsl:choose>
        ...
        <xsl:when test="not(RoomStays)">
          <Errors>
            <Error>
              <xsl:attribute name="Code">1A</xsl:attribute>
              <xsl:value-of select="'No rooms available'"/>
            </Error>
          </Errors>
        </xsl:when>
        <xsl:otherwise>
          ...
          <RoomStays>
            <xsl:choose>
              <xsl:when test="RoomStays/RoomStay/RoomRates/RoomRate/@BookingCode!=''">
                <xsl:apply-templates select="HotelStays/HotelStay" mode="single"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:apply-templates select="HotelStays/HotelStay" mode="multi"/>
              </xsl:otherwise>
            </xsl:choose>
          </RoomStays>
        </xsl:otherwise>
  ...
  <!-- *********************************************************************************************************  -->
  <xsl:template match="HotelStay" mode="single">
    <xsl:variable name="rateref" select="@RoomStayRPH"/>
    ...
    <RoomStay AvailabilityStatus="AvailableForSale">
            ...
            <RoomType>
              <Amenities>
                <xsl:for-each select="$htldesc/FacilityInfo/GuestRooms/GuestRoom[1]/Amenities/Amenity">
                  <Amenity>
                    <xsl:attribute name="RoomAmenity">
                      <xsl:value-of select="concat('RMA',@RoomAmenityCode)"/>
                    </xsl:attribute>
                  </Amenity>
                </xsl:for-each>
              </Amenities>
      ...
      <RatePlans>
        <xsl:call-template name="rateinfo">
          <xsl:with-param name="rateref" select="$rateref"/>
        </xsl:call-template>
        <!--xsl:apply-templates select="../../singleAvailabilityDetails[position() > 1]/propertyAvailability/rateInformation" mode="One"/-->
      </RatePlans>
      <xsl:if test="../../OTA_HotelAvailRS[@EchoToken='Pricing']">
        <RoomRates>
          <xsl:for-each select="../../OTA_HotelAvailRS[@EchoToken='Pricing']">
            <RoomRate>
              <xsl:attribute name="BookingCode">
                <xsl:value-of select="RoomStays/RoomStay/RoomRates/RoomRate/@BookingCode"/>
              </xsl:attribute>
              <Rates>
                <xsl:variable name="baserate">
                  <xsl:value-of select="RoomStays/RoomStay/RoomRates/RoomRate/Rates/Rate[position()=last()]/Total/@AmountBeforeTax"/>
                </xsl:variable>
                <xsl:for-each select="RoomStays/RoomStay/RoomRates/RoomRate/Rates/Rate[@EffectiveDate!='']">
                  ...
                  <Rate>
                    ...
                    <xsl:attribute name="RateTimeUnit">
                      ...
                      <xsl:choose>
                        <xsl:when test="chgOfRateConvertedAmnt/conversionRateDetails/convertedValueAmount!=''">
                          ...
                          <xsl:attribute name="CurrencyCode">
                            <xsl:value-of select="chgOfRateConvertedAmnt/conversionRateDetails/currency"/>
                          </xsl:attribute>
                          <xsl:attribute name="DecimalPlaces">
                            <xsl:choose>
                              <xsl:when test="contains(chgOfRateConvertedAmnt/conversionRateDetails/convertedValueAmount,'.')">
                        ...
                        <xsl:when test="$cur!=''">
                          <xsl:attribute name="AmountBeforeTax">
                            <xsl:value-of select="translate(format-number(Base/@AmountBeforeTax * $currate,'0.00'),'.','')"/>
                          </xsl:attribute>
                          <xsl:attribute name="CurrencyCode">
                        ...
                        <xsl:otherwise>
                          <xsl:attribute name="AmountBeforeTax">
                            <xsl:value-of select="translate(Base/@AmountBeforeTax,'.','')"/>
                          </xsl:attribute>
                          <xsl:attribute name="CurrencyCode">
                            <xsl:value-of select="Base/@CurrencyCode"/>
                          </xsl:attribute>
                          <xsl:attribute name="DecimalPlaces">
                            <xsl:choose>
                              <xsl:when test="contains(Base/@AmountBeforeTax,'.')">
                                <xsl:value-of select="'2'"/>
                              </xsl:when>
                              <xsl:otherwise>
                                <xsl:value-of select="'0'"/>
                    ...
                    <xsl:if test="../../../../RatePlans/RatePlan/CancelPenalties">
                      <CancelPolicies>
                        <CancelPenalty>
                          ...
                          <PenaltyDescription>
                            <xsl:attribute name="Name">
                              <xsl:value-of select="../../../../RatePlans/RatePlan/CancelPenalties/CancelPenalty/@PolicyCode"/>
                            </xsl:attribute>
                            <Text>
                              <xsl:for-each select="../../../../RatePlans/RatePlan/CancelPenalties/CancelPenalty/PenaltyDescription">
                    ...
                    <xsl:if test="$pos=1">
                      <Total>
                        <xsl:variable name="hrate">
                          <xsl:choose>
                            <xsl:when test="../../../../RoomRates/RoomRate/Total/@AmountBeforeTax!=''">
                              <xsl:value-of select="../../../../RoomRates/RoomRate/Total/@AmountBeforeTax"/>
                            </xsl:when>
                            <xsl:when test="../../../../RoomRates/RoomRate/Total/@AmountAfterTax!=''">
                              <xsl:value-of select="../../../../RoomRates/RoomRate/Total/@AmountAfterTax"/>
                            </xsl:when>
                            <xsl:when test="../../../../Total/@AmountBeforeTax!=''">
                              <xsl:value-of select="../../../../Total/@AmountBeforeTax"/>
                            </xsl:when>
                            <xsl:when test="../../../../Total/@AmountAfterTax!=''">
                        ...
                        <xsl:variable name="totalrate">
                          <xsl:choose>
                            <xsl:when test="$cur!=''">
                              <xsl:value-of select="translate(format-number($hrate * $currate,'0.00'),'.','')"/>
                            </xsl:when>
                            <xsl:otherwise>
                              <xsl:value-of select="$hrate"/>
                            </xsl:otherwise>
                          </xsl:choose>
                        </xsl:variable>
                        <xsl:attribute name="AmountBeforeTax">
                          <xsl:value-of select="translate($totalrate,'.','')"/>
                        </xsl:attribute>
                        <xsl:attribute name="CurrencyCode">
                          <xsl:choose>
                            <xsl:when test="$cur!=''">
                            ...
                            <xsl:when test="../../../../RoomRates/RoomRate/Total/@CurrencyCode!=''">
                        ...
                        <xsl:variable name="dec">
                          <xsl:choose>
                            <xsl:when test="$cur!=''">
                              <xsl:value-of select="string-length(substring-after(format-number($totalrate * $currate,'0.00'),'.'))"/>
                            </xsl:when>
                            <xsl:otherwise>
                              <xsl:value-of select="string-length(substring-after($totalrate,'.'))"/>
                            </xsl:otherwise>
                          </xsl:choose>
                        </xsl:variable>
                        <xsl:attribute name="DecimalPlaces">
      ...
      <xsl:variable name="raterph">
        <xsl:choose>
          <xsl:when test="contains($rateref,' ')">
            <xsl:value-of select="substring-before($rateref,' ')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="$rateref"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:variable>
      <GuestCounts IsPerRoom="1">
        <GuestCount>
          <xsl:attribute name="Count">
            <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$raterph]/GuestCounts/GuestCount/@Count"/>
          </xsl:attribute>
        </GuestCount>
      </GuestCounts>
      <TimeSpan>
        <xsl:attribute name="Start">
          <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$raterph]/TimeSpan/@Start"/>
        </xsl:attribute>
        <!--xsl:variable name="v1" select="ttVB:FctDateDuration(string(../../RoomStays/RoomStay[@RPH=$rateref]/TimeSpan/@Start),string(../../RoomStays/RoomStay[@RPH=$rateref]/TimeSpan/@End))"/>
				<xsl:attribute name="Duration"><xsl:value-of select="$v1"/></xsl:attribute-->
        <xsl:attribute name="End">
          <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$raterph]/TimeSpan/@End"/>
        </xsl:attribute>
      </TimeSpan>
      <xsl:if test="../../OTA_HotelDescriptiveInfoRS/HotelDescriptiveContents/HotelDescriptiveContent[@HotelCode=$hoteldesc]">
        <xsl:variable name="htldesc" select="../../OTA_HotelDescriptiveInfoRS/HotelDescriptiveContents/HotelDescriptiveContent[@HotelCode=$hoteldesc]"/>
      ...
      <Total>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:attribute name="CurrencyCode">
              <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$raterph]/RoomRates/RoomRate/Total/@CurrencyCode"/>
            </xsl:attribute>
            <xsl:variable name="rcur">
              <xsl:choose>
                <xsl:when test="../../RoomStays/RoomStay[@RPH=$raterph]/RoomRates/RoomRate/Total/@AmountBeforeTax">
                  <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$raterph]/RoomRates/RoomRate/Total/@AmountBeforeTax"/>
                </xsl:when>
                <xsl:when test="../../RoomStays/RoomStay[@RPH=$raterph]/RoomRates/RoomRate/Total/@AmountAfterTax">
                  <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$raterph]/RoomRates/RoomRate/Total/@AmountAfterTax"/>
                </xsl:when>
              </xsl:choose>
            </xsl:variable>
            <xsl:attribute name="DecimalPlaces">
              <xsl:choose>
                <xsl:when test="contains($rcur,'.')">2</xsl:when>
      ...
      <BasicPropertyInfo>
        ...
        <xsl:attribute name="HotelCode">
          <xsl:value-of select="substring(BasicPropertyInfo/@HotelCode,6)"/>
        </xsl:attribute>
        ...
        <xsl:attribute name="ChainName">
          <xsl:value-of select="BasicPropertyInfo/@ChainName"/>
        </xsl:attribute>
        <xsl:attribute name="HotelCodeContext">
          <xsl:value-of select="BasicPropertyInfo/@HotelCodeContext"/>
        </xsl:attribute>
        <xsl:apply-templates select="//HotelsRS/Hotel_FeaturesReply[BasicPropertyInfo/ChainCode = $chcode]"/>
        <xsl:if test="../../OTA_HotelDescriptiveInfoRS/HotelDescriptiveContents/HotelDescriptiveContent[@HotelCode=$hoteldesc]">
          <xsl:variable name="htldesc" select="../../OTA_HotelDescriptiveInfoRS/HotelDescriptiveContents/HotelDescriptiveContent[@HotelCode=$hoteldesc]"/>
          <VendorMessages>
            <VendorMessage>
              ...
              <SubSection SubTitle="HotelMainImage">
                <Paragraph Name="{BasicPropertyInfo/@HotelName}">
                  <URL>
                    <xsl:choose>
              ...
              <SubSection SubTitle="HotelMainText">
                <Paragraph>
                  <xsl:choose>
                    ...
                    <xsl:otherwise>
                      <Text>
                        <xsl:value-of select="concat('A ',BasicPropertyInfo/@ChainName,' property')"/>
                      </Text>
                    </xsl:otherwise>
                  </xsl:choose>
                </Paragraph>
              ...
              <xsl:for-each select="$htldesc/HotelInfo/Descriptions/MultimediaDescriptions/MultimediaDescription[@AdditionalDetailCode!='3' or not(@AdditionalDetailCode)][not(ImageItems)]">
                <SubSection>
                  ...
                  <xsl:attribute name="SubCode">
                    <xsl:choose>
                      <xsl:when test="@AdditionalDetailCode!=''">
                        <xsl:value-of select="concat('ADT',@AdditionalDetailCode)"/>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="concat('INF',@InfoCode)"/>
                      </xsl:otherwise>
                    </xsl:choose>
                  </xsl:attribute>
                  <Paragraph>
            ...
            <xsl:choose>
              <xsl:when test="$htldesc/Policies/Policy/PolicyInfo/@CheckInTime!=''">
                <VendorMessage>
                  <SubSection>
                    <xsl:attribute name="SubTitle">
                      <xsl:value-of select="'Check-in Time'"/>
                    </xsl:attribute>
                    <Paragraph>
                      <Text>
                  ...
                  <xsl:if test="$htldesc/Policies/Policy/PolicyInfo/@CheckOutTime!=''">
                    <SubSection>
                      <xsl:attribute name="SubTitle">
                        <xsl:value-of select="'Check-out Time'"/>
                      </xsl:attribute>
                      <Paragraph>
                        <Text>
            ...
            <xsl:for-each select="$htldesc/ContactInfos/ContactInfo[1]/Phones/Phone">
              <ContactNumber>
                <xsl:attribute name="PhoneTechType">
                  <xsl:value-of select="@PhoneTechType"/>
                </xsl:attribute>
        ...
        <RelativePosition>
          <Transportations>
            <xsl:if test="BasicPropertyInfo/bestTransportation !=''">
              <Transportation>
                <xsl:attribute name="TransportationCode">
                  <xsl:value-of select="BasicPropertyInfo/bestTransportation"/>
                </xsl:attribute>
              </Transportation>
            </xsl:if>
              ...
              <Transportation>
                <Descriptions>
                  <Description Name="Description">
                    <xsl:apply-templates select="//HotelsRS/Hotel_FeaturesReply[BasicPropertyInfo/ChainCode = $chcode]/hotelFeaturesTerms/featuresTerms[category='1']/description"/>
                    <xsl:apply-templates select="//HotelsRS/Hotel_FeaturesReply[BasicPropertyInfo/ChainCode = $chcode]/hotelFeaturesTerms/featuresTerms[category='11A']/description"/>
                    <xsl:apply-templates select="//HotelsRS/Hotel_FeaturesReply[BasicPropertyInfo/ChainCode = $chcode]/hotelFeaturesTerms/featuresTerms[category='11']/description"/>
                  </Description>
                  <Description Name="Facilities">
                    <xsl:apply-templates select="//HotelsRS/Hotel_FeaturesReply[BasicPropertyInfo/ChainCode = $chcode]/hotelFeaturesTerms/featuresTerms[category='4']/description"/>
                    <xsl:apply-templates select="//HotelsRS/Hotel_FeaturesReply[BasicPropertyInfo/ChainCode = $chcode]/hotelFeaturesTerms/featuresTerms[category='4A']/description"/>
                  </Description>
                </Descriptions>
              </Transportation>
            </xsl:if>
                ...
                <xsl:when test="Features/Feature">
                  <xsl:for-each select="Features/Feature">
                    <HotelAmenity>
                      <xsl:attribute name="Code">
                        <xsl:choose>
                          <xsl:when test="@AccessibleCode!=''">
                            <xsl:value-of select="concat('PHY',@AccessibleCode)"/>
                          </xsl:when>
                          <xsl:when test="@SecurityCode!=''">
                            <xsl:value-of select="concat('SEC',@SecurityCode)"/>
                          </xsl:when>
                          <xsl:otherwise>
                            <xsl:value-of select="."/>
                          </xsl:otherwise>
      ...
      <AlternateInfo>
        <xsl:choose>
          <xsl:when test="BasicPropertyInfo/@HotelCodeContext = 'CA'">Complete Access</xsl:when>
          <xsl:when test="BasicPropertyInfo/@HotelCodeContext = 'CP'">Complete Access Plus</xsl:when>
          <xsl:when test="BasicPropertyInfo/@HotelCodeContext = 'DY'">Dynamic Access</xsl:when>
          <xsl:when test="BasicPropertyInfo/@HotelCodeContext = 'IA'">Independent Access</xsl:when>
          <xsl:when test="BasicPropertyInfo/@HotelCodeContext = 'SA'">Standard Access</xsl:when>
        </xsl:choose>
      </AlternateInfo>
    </RoomStay>
  </xsl:template>

  <xsl:template name="rateinfo">
    <xsl:param name="rateref"/>
    <xsl:if test="$rateref!=''">
      <xsl:variable name="raterph">
        <xsl:choose>
          <xsl:when test="contains($rateref,' ')">
      ...
      <xsl:apply-templates select="../../RoomStays/RoomStay[@RPH=$raterph]" mode="One"/>
      <xsl:call-template name="rateinfo">
        <xsl:with-param name="rateref" select="substring-after($rateref,' ')"/>
      </xsl:call-template>
    </xsl:if>
  </xsl:template>

  <xsl:template match="HotelStay" mode="multi">
    <xsl:variable name="rateref" select="@RoomStayRPH"/>
    <xsl:variable name="hoteldesc">
      <xsl:value-of select="BasicPropertyInfo/@HotelCode"/>
    </xsl:variable>
    <RoomStay AvailabilityStatus="AvailableForSale">
      ...
      <RatePlans>
        <RatePlan>
          <AdditionalDetails>
            <xsl:apply-templates select="../../RoomStays/RoomStay[@RPH=$rateref]/RoomRates/RoomRate/Rates/Rate[@RateMode='1']"/>
            <xsl:apply-templates select="../../RoomStays/RoomStay[@RPH=$rateref]/RoomRates/RoomRate/Rates/Rate[@RateMode='2']"/>
          </AdditionalDetails>
        </RatePlan>
      </RatePlans>
      <GuestCounts IsPerRoom="1">
        <GuestCount>
          <xsl:attribute name="Count">
            <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$rateref]/GuestCounts/GuestCount/@Count"/>
          </xsl:attribute>
        </GuestCount>
      </GuestCounts>
      <TimeSpan>
        <xsl:attribute name="Start">
          <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$rateref]/TimeSpan/@Start"/>
        </xsl:attribute>
        <!--xsl:variable name="v1" select="ttVB:FctDateDuration(string(../../RoomStays/RoomStay[@RPH=$rateref]/TimeSpan/@Start),string(../../RoomStays/RoomStay[@RPH=$rateref]/TimeSpan/@End))"/>
				<xsl:attribute name="Duration"><xsl:value-of select="$v1"/></xsl:attribute-->
        <xsl:attribute name="End">
          <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$rateref]/TimeSpan/@End"/>
        </xsl:attribute>
      </TimeSpan>
      <Total>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:attribute name="CurrencyCode">
              <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$rateref]/RoomRates/RoomRate/Rates/Rate[@RateMode='1']/Base/@CurrencyCode"/>
            </xsl:attribute>
            <xsl:variable name="rcur">
              <xsl:choose>
                <xsl:when test="../../RoomStays/RoomStay[@RPH=$rateref]/RoomRates/RoomRate/Rates/Rate[@RateMode='2']/Base/@AmountBeforeTax">
                  <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$rateref]/RoomRates/RoomRate/Rates/Rate[@RateMode='2']/Base/@AmountBeforeTax"/>
                </xsl:when>
                <xsl:when test="../../RoomStays/RoomStay[@RPH=$rateref]/RoomRates/RoomRate/Rates/Rate[@RateMode='2']/Base/@AmountAfterTax">
                  <xsl:value-of select="../../RoomStays/RoomStay[@RPH=$rateref]/RoomRates/RoomRate/Rates/Rate[@RateMode='2']/Base/@AmountAfterTax"/>
                </xsl:when>
              </xsl:choose>
            </xsl:variable>
            <xsl:attribute name="DecimalPlaces">
      ...
      <BasicPropertyInfo>
        ...
        <RelativePosition>
          <Transportations>
            <xsl:if test="BasicPropertyInfo/RelativePosition/Transportations/Transportation/@TransportationCode!=''">
              <Transportation>
                <xsl:attribute name="TransportationCode">
                  <xsl:value-of select="BasicPropertyInfo/RelativePosition/Transportations/Transportation/@TransportationCode"/>
                </xsl:attribute>
              </Transportation>
            </xsl:if>
            <Transportation>
							...
							<Descriptions>
								<Description Name="Photo">
									<URL>
										...
										<xsl:value-of select="BasicPropertyInfo/HotelCityCode" />
										<xsl:value-of select="BasicPropertyInfo/HotelCode" />/<xsl:value-of select="$chcode" />
										<xsl:value-of select="BasicPropertyInfo/HotelCityCode" />
										<xsl:value-of select="BasicPropertyInfo/HotelCode" />
      ...
      <AlternateInfo>
        <xsl:choose>
          ...
          <xsl:when test="BasicPropertyInfo/@HotelCodeContext = 'IA'">Independent Access</xsl:when>
          <xsl:when test="BasicPropertyInfo/@HotelCodeContext = 'SA'">Standard Access</xsl:when>
          <xsl:when test="BasicPropertyInfo/@HotelCodeContext = '1A'">Amadeus Access</xsl:when>
        </xsl:choose>
      </AlternateInfo>
    </RoomStay>
  </xsl:template>
  <!-- *********************************************************************************************************  -->
  <!-- *********************************************************************************************************  -->
  <xsl:template match="RoomStay" mode="One">
    <RatePlan>
      <xsl:attribute name="BookingCode">
        <xsl:value-of select="RoomRates/RoomRate/@BookingCode"/>
      </xsl:attribute>
      <xsl:attribute name="RatePlanCode">
        <xsl:value-of select="RoomRates/RoomRate/@RatePlanCode"/>
      </xsl:attribute>
      <xsl:if test="roomInformation/rateCategoryCode != ''">
        <xsl:attribute name="RatePlanType">
          <xsl:value-of select="roomInformation/rateCategoryCode"/>
        </xsl:attribute>
      </xsl:if>
      <xsl:if test="count(RoomRates/RoomRate/Rates/Rate) > 1">
        <xsl:attribute name="RateIndicator">ChangeDuringStay</xsl:attribute>
      </xsl:if>
      <xsl:if test="RatePlans/RatePlan/Guarantee/@GuaranteeType">
        <Guarantee>
          <xsl:attribute name="GuaranteeType">
            <xsl:value-of select="RatePlans/RatePlan/Guarantee/@GuaranteeType"/>
          </xsl:attribute>
        </Guarantee>
      </xsl:if>
      <xsl:if test="RatePlans/RatePlan/Guarantee/@HoldTime">
        <Guarantee>
          <xsl:attribute name="HoldTime">
            <xsl:value-of select="RatePlans/RatePlan/Guarantee/@HoldTime"/>
          </xsl:attribute>
        </Guarantee>
      </xsl:if>
      <xsl:if test="rateIndicators/mealIncluded='B'">
        <MealsIncluded>
          <xsl:attribute name="Breakfast">
      ...
      <RestrictionStatus>
        <xsl:choose>
          <xsl:when test="@AvailabilityStatus='AvailableForSale'">
      ...
      <AdditionalDetails>
        <AdditionalDetail>
          <xsl:attribute name="Amount">
            <xsl:variable name="base">
              <xsl:choose>
                <xsl:when test="RoomRates/RoomRate/Rates/Rate/Base/@AmountBeforeTax">
                  <xsl:value-of select="format-number(RoomRates/RoomRate/Rates/Rate/Base/@AmountBeforeTax,'0.00')"/>
                </xsl:when>
                <xsl:when test="RoomRates/RoomRate/Rates/Rate/Base/@AmountAfterTax">
                  <xsl:value-of select="format-number(RoomRates/RoomRate/Rates/Rate/Base/@AmountAfterTax,'0.00')"/>
                </xsl:when>
              </xsl:choose>
            </xsl:variable>
            <xsl:choose>
              <xsl:when test="$cur!=''">
                <xsl:value-of select="translate(format-number($base * $currate,'0.00'),'.','')"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="translate(format-number($base,'0.00'),'.','')"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:attribute>
          <xsl:if test="RoomRates/RoomRate/RoomRateDescription/Text!=''">
            <DetailDescription>
              <xsl:for-each select="RoomRates/RoomRate/RoomRateDescription/Text">
                <Text>
                  <xsl:value-of select="."/>
                </Text>
              </xsl:for-each>
        ...
        <xsl:variable name="cat">
          <xsl:value-of select="substring(RoomRates/RoomRate/@RoomTypeCode,1,1)"/>
        </xsl:variable>
        <xsl:variable name="nip">
          <xsl:value-of select="substring(RoomRates/RoomRate/@RoomTypeCode,2,1)"/>
        </xsl:variable>
        <xsl:variable name="bed">
          <xsl:value-of select="substring(RoomRates/RoomRate/@RoomTypeCode,3,1)"/>
        </xsl:variable>
        ...
        <AdditionalDetail Type="NumberOfBeds">
          <DetailDescription>
            <xsl:attribute name="Name">
              <xsl:choose>
                <xsl:when test="translate($nip,'1234567890','')!=''">
                  <xsl:value-of select="'Any'"/>
                </xsl:when>
                <xsl:otherwise>
  ...
  <xsl:template match="rateInformation" mode="One">
    <RatePlan>
      <xsl:attribute name="BookingCode">
        <xsl:choose>
          <xsl:when test="rateReferenceDetails/providerBookingCode!= ''">
            <xsl:value-of select="rateReferenceDetails/providerBookingCode"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="roomInformation/roomType"/>
            <xsl:value-of select="roomInformation/rateCode"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:attribute>
      <xsl:attribute name="RatePlanCode">
      ...
      <xsl:choose>
        ...
        <xsl:when test="rateIndicators/guaranteeRequired != ''">
          <Guarantee>
            <xsl:attribute name="HoldTime">
              <xsl:value-of select="rateIndicators/guaranteeRequired + 12"/>:00:00
            </xsl:attribute>
          </Guarantee>
        </xsl:when>
      </xsl:choose>
    ...
    <RoomStay AvailabilityStatus="AvailableForSale">
      ...
      <xsl:if test="hotelPricingCategorySection[pricingCategory/itemDescriptionType='DES']/roomRateInfoSection">
        ...
        <RoomRates>
          <RoomRate>
            <xsl:attribute name="BookingCode">
              <xsl:value-of select="$roomrate/roomInformation/bookingCode"/>
            </xsl:attribute>
            <xsl:attribute name="RoomTypeCode">
              <xsl:value-of select="$roomrate/roomInformation/roomRateIdentifier/roomType"/>
            </xsl:attribute>
            <xsl:attribute name="RatePlanCode">
              <xsl:value-of select="$roomrate/roomInformation/roomRateIdentifier/ratePlanCode"/>
            </xsl:attribute>
                        ...
                        <Fee>
                          ...
                          <Description>
                            ...
                            <xsl:if test="perPerson='PER'">
                              <Text>
                                <xsl:value-of select="'Per Person'"/>
                              </Text>
                            </xsl:if>
                            <xsl:if test="perPerson='ROO'">
                              <Text>
                                <xsl:value-of select="'Per Room'"/>
                              </Text>
                            </xsl:if>
                            ...
                            <xsl:if test="timeUnit='PER'">
                              <Text>
                                <xsl:value-of select="'Per period'"/>
                              </Text>
                            </xsl:if>
                ...
                <GuaranteeAccepted>
                  <PaymentCard>
                    <xsl:attribute name="CardCode">
                      <xsl:choose>
                        <xsl:when test="formOfPayment/vendorCode='CA'">
                          <xsl:value-of select="'MC'"/>
                        </xsl:when>
                        <xsl:when test="formOfPayment/vendorCode='MA'">
                          <xsl:value-of select="'MC'"/>
                        </xsl:when>
                        <xsl:when test="formOfPayment/vendorCode='DC'">
                          <xsl:value-of select="'DN'"/>
                        </xsl:when>
                        <xsl:when test="formOfPayment/vendorCode='AM'">
                          <xsl:value-of select="'AX'"/>
                        </xsl:when>
                        <xsl:otherwise>
                          <xsl:value-of select="formOfPayment/vendorCode"/>
                        </xsl:otherwise>
                      </xsl:choose>
                    </xsl:attribute>
                  </PaymentCard>
                ...
                <Deadline>
                  <xsl:attribute name="OffsetTimeUnit">
                    ...
                    <xsl:choose>
                      ...
                      <xsl:when test="$offset='3'">
                        <xsl:value-of select="'Day'"/>
                      </xsl:when>
                      <xsl:when test="$offset='4'">
                        <xsl:value-of select="'Week'"/>
                      </xsl:when>
                      <xsl:when test="$offset='5'">
      ...
      <xsl:if test="hotelPricingCategorySection[pricingCategory/itemDescriptionType='CXL']/infoMsgAndCancelPolicies/freeText!=''">
        <CancelPenalties>
          <CancelPenalty>
            ...
            <PenaltyDescription>
              <xsl:attribute name="Name">
                <xsl:value-of select="hotelPricingCategorySection[pricingCategory/itemDescriptionType='CXL']/pricingCategory/itemDescription/description"/>
              </xsl:attribute>
              <Text>
          ...
          <Taxes>
            <xsl:for-each select="hotelPricingCategorySection[pricingCategory/itemDescriptionType='INC']/taxSection[taxFeeInformation/category='TAX'] | hotelPricingCategorySection[pricingCategory/itemDescriptionType='TTX']/taxSection[taxFeeInformation/category='SCH'] | hotelPricingCategorySection[pricingCategory/itemDescriptionType='TTX']/taxSection[taxFeeInformation/category='TAX']">
              <Tax>
                ...
                <xsl:attribute name="Code">
                  <xsl:value-of select="taxFeeInformation/code"/>
                </xsl:attribute>
                <xsl:choose>
                  <xsl:when test="taxFeeInformation/percentage!=''">
                    <xsl:attribute name="Percent">
                      <xsl:value-of select="taxFeeInformation/percentage"/>
                    </xsl:attribute>
                  </xsl:when>
                  <xsl:when test="taxFeeInformation/amount!=''">
                    <xsl:attribute name="Amount">
                      <xsl:value-of select="translate(taxFeeInformation/amount,'.','')"/>
                    </xsl:attribute>
                  </xsl:when>
                </xsl:choose>
      ...
      <BasicPropertyInfo>
        ...
        <xsl:attribute name="HotelCityCode">
          <xsl:value-of select="hotelPropertyInfo/hotelReference/HotelCityCode"/>
        </xsl:attribute>
        <xsl:attribute name="HotelName">
          ...
          <VendorMessages>
            <VendorMessage>
              ...
              <xsl:if test="$other/freeText!=''">
                <SubSection>
                  <xsl:attribute name="SubTitle">
                    <xsl:value-of select="'Other Information'"/>
                  </xsl:attribute>
                  <Paragraph>
  ...
  <xsl:template match="Rate">
    <AdditionalDetail>
      <xsl:attribute name="Code">
        <xsl:choose>
          <xsl:when test="@RateMode='1'">MinimumRate</xsl:when>
          <xsl:otherwise>MaximumRate</xsl:otherwise>
        </xsl:choose>
      </xsl:attribute>
      <xsl:attribute name="Amount">
        <xsl:variable name="base">
          <xsl:choose>
            <xsl:when test="Base/@AmountBeforeTax">
              <xsl:value-of select="format-number(Base/@AmountBeforeTax,'0.00')"/>
            </xsl:when>
            <xsl:when test="Base/@AmountAfterTax">
              <xsl:value-of select="format-number(Base/@AmountAfterTax,'0.00')"/>
            </xsl:when>
          </xsl:choose>
        </xsl:variable>
        <xsl:choose>
  ...
  <xsl:template match="Hotel_FeaturesReply">
      ...
      <xsl:if test="hotelFeaturesTerms/featuresTerms[category='1D']/description != ''">
        <StateProv>
          <xsl:attribute name="StateCode">
            <xsl:value-of select="hotelFeaturesTerms/featuresTerms[category='1D']/description"/>
          </xsl:attribute>
        </StateProv>
      </xsl:if>
      <CountryName>
    ...
    <ContactNumbers>
      <ContactNumber>
        ...
        <xsl:attribute name="PhoneNumber">
          <xsl:value-of select="hotelFeaturesTerms/featuresTerms[category='1G']/description"/>
        </xsl:attribute>
      </ContactNumber>
      <ContactNumber>
        <xsl:attribute name="PhoneTechType">Fax</xsl:attribute>
        <xsl:attribute name="PhoneNumber">
          <xsl:value-of select="hotelFeaturesTerms/featuresTerms[category='1I']/description"/>
        </xsl:attribute>
      </ContactNumber>
    </ContactNumbers>
  </xsl:template>
