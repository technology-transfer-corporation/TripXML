...
<xsl:template match="/">
	<xsl:apply-templates select="PNR_PNRReply" />
	<xsl:apply-templates select="MessagesOnly_Reply" />
	<xsl:apply-templates select="Command_CrypticReply" />
</xsl:template>

<xsl:template match="MessagesOnly_Reply | Command_CrypticReply">
...
<xsl:template match="longTextString">
	<Error>
		<xsl:attribute name="Type">Amadeus</xsl:attribute>
		<xsl:value-of select="textStringDetails" />
	</Error>
</xsl:template>

<xsl:template match="PNR_Reply">
	<OTA_TravelItineraryRS>
		<xsl:attribute name="Version"><xsl:value-of select="'2.000'"/></xsl:attribute>
		<xsl:choose>
