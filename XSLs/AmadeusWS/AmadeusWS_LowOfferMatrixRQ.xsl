...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ttVB="urn:ttVB" exclude-result-prefixes="msxsl ttVB" version="1.0">
	<!-- ================================================================== -->
	<!-- AmadeusWS_LowOfferMatrixRQ.xsl 												-->
	<!-- ================================================================== -->
	<!-- Date: 30 Sep 2012 - Rastko - new file												-->
	<!-- ================================================================== -->
	<xsl:output method="xml" omit-xml-declaration="yes" />
	<xsl:variable name="pcc" select="OTA_AirLowOfferMatrixRQ/POS/Source/@PseudoCityCode"/>
	<xsl:template match="/">
		<xsl:apply-templates select="OTA_AirLowOfferMatrixRQ" />
	</xsl:template>
	<!-- ======================================================================= -->
	<xsl:template match="OTA_AirLowOfferMatrixRQ">
		<Fare_SellByFareCalendar>
			...
			<fareFamilies>
				<familyInformation>
					<fareFamilyname><xsl:value-of select="TravelerInfoSummary/PriceRequestInformation/NegotiatedFareCode/@Code"/></fareFamilyname>
					<hierarchy>100</hierarchy>
				</familyInformation>
				<familyCriteria>
					<carrierId>
						<xsl:value-of select="TravelPreferences/VendorPref/@Code"/>
					</carrierId>
					<fareFamilyInfo>
						<fareFamilyQual>NCO</fareFamilyQual>
					</fareFamilyInfo>
					<fareProductDetail>
						<fareBasis><xsl:value-of select="TravelerInfoSummary/PriceRequestInformation/NegotiatedFareCode/@Code"/></fareBasis>
						<xsl:choose>
							<xsl:when test="TravelerInfoSummary/PriceRequestInformation[not(NegotiatedFareCode/@Code)]/@PricingSource='Private'">
								<fareType>RU</fareType>
							</xsl:when>
							<xsl:otherwise>
