...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" version="1.0">
	...
	<!-- ================================================================== -->
		<!-- Date: 16 Mar 2012 -  shashin - removed ITF > INF conversion		-->
	<!-- Date: 12 Mar 2012 -  shashin - corrected logic for NumberInParty to avoid ITF		-->
	
	<!-- Date: 31 Aug 2011 -  Rastko - corrected logic for processing second OD			-->
	<!-- Date: 13 Aug 2011 -  Rastko - mapped cabin									-->
	<!-- Date: 10 Aug 2011 - Rastko - corrected error processing when no more flights returned	-->
	<!-- Date: 19 Jul 2011 - Rastko - corrected cabin mapping							-->
	...
	<xsl:template match="/">
		<xsl:choose>
			...
			<xsl:when test="FS/AirAvail/Air_MultiAvailabilityReply/singleCityPairInfo/cityPairErrorOrWarning/cityPairErrorOrWarningText[freeTextQualification/codedIndicator='3'] and not(FS/AirAvail/Air_MultiAvailabilityReply/singleCityPairInfo/flightInfo/basicFlightInfo/flightDetails)">
				<OTA_AirLowFareSearchFlightsRS Version="1.001">
					<xsl:attribute name="TransactionIdentifier">Amadeus</xsl:attribute>
					<Errors>
			...
			<xsl:when test="FS/Fare_InformativePricingWithoutPNRReply/Errors">
				<OTA_AirLowFareSearchFlightsRS Version="1.001">
					<xsl:attribute name="TransactionIdentifier">Amadeus</xsl:attribute>
					<Errors>
						<xsl:apply-templates select="FS/Fare_InformativePricingWithoutPNRReply/Errors/Error" mode="Error" />
					</Errors>
				</OTA_AirLowFareSearchFlightsRS>
			</xsl:when>
			<xsl:when test="FS/Fare_InformativeBestPricingWithoutPNRReply/errorGroup/errorMessage/freeText!=''">
				<OTA_AirLowFareSearchFlightsRS Version="1.001">
					<xsl:attribute name="TransactionIdentifier">Amadeus</xsl:attribute>
					<Errors>
						<xsl:apply-templates select="FS/Fare_InformativeBestPricingWithoutPNRReply/errorGroup/errorMessage" mode="Error" />
					</Errors>
				</OTA_AirLowFareSearchFlightsRS>
			</xsl:when>
			<xsl:when test="FS/Fare_InformativeBestPricingWithoutPNRReply/Errors">
				<OTA_AirLowFareSearchFlightsRS Version="1.001">
					<xsl:attribute name="TransactionIdentifier">Amadeus</xsl:attribute>
					<Errors>
						<xsl:apply-templates select="FS/Fare_InformativeBestPricingWithoutPNRReply/Errors/Error" mode="Error" />
					</Errors>
				</OTA_AirLowFareSearchFlightsRS>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="Air_MultiAvailabilityReply">
		...
		<xsl:variable name="flights">
			<OriginDestinationOption>
				<xsl:attribute name="SectorSequence"><xsl:value-of select="$posOD"/></xsl:attribute>
				<xsl:for-each select="singleCityPairInfo/flightInfo[basicFlightInfo/productTypeDetail/productIndicators='S' or basicFlightInfo/productTypeDetail/productIndicators='D'][position() &lt; $pos]">
					...
					<xsl:if test="basicFlightInfo/productTypeDetail/productIndicators = 'S'">
						<xsl:apply-templates select="following-sibling::flightInfo[1]" mode="conx">
							<xsl:with-param name="pot"><xsl:value-of select="$pot + 1"/></xsl:with-param>
							<xsl:with-param name="rph"><xsl:value-of select="position()"/></xsl:with-param>
							<xsl:with-param name="pos" select="$pos"/>
						</xsl:apply-templates>
					</xsl:if>
		...
		<xsl:apply-templates select="following-sibling::Air_MultiAvailabilityReply[1]">
			<xsl:with-param name="pot"><xsl:value-of select="$pot + count(msxsl:node-set($flights)/OriginDestinationOption/FlightSegment)"/></xsl:with-param>
			<xsl:with-param name="pos" select="$pos"/>
			<xsl:with-param name="posOD"><xsl:value-of select="$posOD + 1"/></xsl:with-param>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template match="flightInfo" mode="leg">
		...
		<FlightSegment>
			...
			<xsl:variable name="cabin">
				<xsl:choose>
					<xsl:when test="../../../../Fare_InformativeBestPricingWithoutPNRReply/mainGroup/pricingGroupLevelGroup[position()=last()]/fareInfoGroup/segmentLevelGroup[position()=$pot]/cabinGroup/cabinSegment/bookingClassDetails/option!=''">
						<xsl:value-of select="../../../../Fare_InformativeBestPricingWithoutPNRReply/mainGroup/pricingGroupLevelGroup[position()=last()]/fareInfoGroup/segmentLevelGroup[position()=$pot]/cabinGroup/cabinSegment/bookingClassDetails/option" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="../../../../Fare_InformativeBestPricingWithoutPNRReply/mainGroup/pricingGroupLevelGroup[position()=last()]/fareInfoGroup/segmentLevelGroup[1]/cabinGroup/cabinSegment/bookingClassDetails/option" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:attribute name="ResBookDesigCode">
				<xsl:choose>
					<xsl:when test="$pos='2'">
						<xsl:value-of select="../../../../Fare_InformativeBestPricingWithoutPNRReply/mainGroup/pricingGroupLevelGroup[position()=last()]/fareInfoGroup/segmentLevelGroup[position()=$pot]/segmentInformation/flightIdentification/bookingClass" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
							<xsl:when test="$cabin='M'">Y</xsl:when>
							<xsl:otherwise><xsl:value-of select="$cabin"/></xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
			<xsl:attribute name="NumberInParty"><xsl:value-of select="sum(../../../../Fare_InformativeBestPricingWithoutPNRReply/AirTravelerAvail/PassengerTypeQuantity[@Code!= 'INF' and @Code!= 'IN' and @Code!= 'ITF']/@Quantity)" /></xsl:attribute>
			<xsl:attribute name="E_TicketEligibility">
				<xsl:choose>
					<xsl:when test="../../../../Fare_InformativeBestPricingWithoutPNRReply/mainGroup/pricingGroupLevelGroup[position()=1]/fareInfoGroup/textData[contains(freeText,'FARE VALID FOR E TICKET ONLY')]">Eligible</xsl:when>
	...
	<xsl:template match="Fare_InformativeBestPricingWithoutPNRReply">
			...
			<PTC_FareBreakdowns>
				<xsl:for-each select="AirTravelerAvail/PassengerTypeQuantity">
					...
					<xsl:apply-templates select="../../mainGroup/pricingGroupLevelGroup[position()=$pos]" mode="paxtypes">
						<xsl:with-param name="paxtype"><xsl:value-of select="$paxtype"/></xsl:with-param>
						<xsl:with-param name="nip"><xsl:value-of select="@Quantity"/></xsl:with-param>
					</xsl:apply-templates>
				</xsl:for-each>
			</PTC_FareBreakdowns>
			<!--FareInfos>
	...
	<xsl:template match="segmentLevelGroup" mode="fareinfo">
		...
		<FareInfo>
			...
			<FareReference><xsl:value-of select="fareBasis/additionalFareDetails/secondRateClass"/></FareReference> 
