...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<!-- ================================================================== -->
	<!-- Date: 02 Apr 2103 -  Rastko - added BreakPoint attribute info to FareInfo element		-->
	<!-- Date: 30 Sep 2012 -  Rastko - fixed alternate ValidatingAirline mapping			-->
	<!-- Date: 27 Sep 2012 -  Rastko - added case when multiple pax types have same fare		-->
	<!-- Date: 03 Sep 2012 - Rastko - corrected mapping of validating airline			 -->
	...
	<!-- Date: 21 Feb 2011 -  Rastko - mapped Notes to all passenger types			-->
	<!-- Date: 13 Feb 2011 -  Rastko - collect fare component in Notes element			-->
	<!-- Date: 29 Sep 2010 -  Rastko - fixed calculation of total base and total fare			-->
	<!-- Date: 13 Aug 2010 -  Rastko - corrected calculation with equiv base fare			-->
	...
	<xsl:variable name="loop">
		<xsl:choose>
			<xsl:when test="//pricingGroupLevelGroup/warningInformation/warningText/errorFreeText = 'LOWEST SOLD OUT//TRY WAIT LIST'">
				<xsl:value-of select="(count(//pricingGroupLevelGroup[numberOfPax]) div 2) + 1"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="count(//pricingGroupLevelGroup[numberOfPax]) + 1"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
						...
						<PricedItinerary>
							...
							<AirItineraryPricingInfo>
								...
								<xsl:if test="mainGroup/pricingGroupLevelGroup[1]/fareInfoGroup/segmentLevelGroup[1]/additionalInformation/idNumber!=''">
									<xsl:attribute name="NegotiatedFareCode"><xsl:value-of select="mainGroup/pricingGroupLevelGroup[1]/fareInfoGroup/corporateGroup/corporateData/locationCode"/></xsl:attribute>
								</xsl:if>
								<ItinTotalFare>
								...
								<PTC_FareBreakdowns>
									<xsl:for-each select="AirTravelerAvail/PassengerTypeQuantity">
										...
										<xsl:apply-templates select="../../mainGroup/pricingGroupLevelGroup[numberOfPax and position()=$pos]" mode="paxtypes">
								...
								<FareInfos>
									<xsl:variable name="va">
										<xsl:choose>
											<xsl:when test="mainGroup/pricingGroupLevelGroup[1]/fareInfoGroup/textData[contains(freeText,'PRICED WITH VALIDATING CARRIER')]/freeText">
												<xsl:variable name="va1">
													<xsl:value-of select="mainGroup/pricingGroupLevelGroup[1]/fareInfoGroup/textData[contains(freeText,'PRICED WITH VALIDATING CARRIER')]/freeText"/>
												</xsl:variable>
												<xsl:value-of select="substring(substring-after($va1,'PRICED WITH VALIDATING CARRIER '),1,2)"/>
											</xsl:when>
											<xsl:when test="mainGroup/pricingGroupLevelGroup[1]/fareInfoGroup/textData[contains(freeText,'OTHER VC AVAILABLE')]/freeText">
												<xsl:variable name="va1">
													<xsl:value-of select="mainGroup/pricingGroupLevelGroup[1]/fareInfoGroup/textData[contains(freeText,'OTHER VC AVAILABLE')]/freeText"/>
												</xsl:variable>
												<xsl:value-of select="substring(substring-after($va1,'PRICED VC '),1,2)"/>
											</xsl:when>
										</xsl:choose>
									</xsl:variable>
									<xsl:variable name="bp" select="BreakPoint"/>
									<xsl:apply-templates select="mainGroup/pricingGroupLevelGroup[1]/fareInfoGroup/segmentLevelGroup" mode="fareinfo">
										<xsl:with-param name="va"><xsl:value-of select="$va"/></xsl:with-param>
										<xsl:with-param name="bp"><xsl:value-of select="$bp"/></xsl:with-param>
									</xsl:apply-templates>
								</FareInfos>
							</AirItineraryPricingInfo>
	...
	<xsl:template match="fareInfoGroup" mode="totalbase">
		...
		<xsl:choose>
			<xsl:when test="($pos &lt; $loop) and ../../pricingGroupLevelGroup[numberOfPax][$pos + 1]/fareInfoGroup">
				<xsl:apply-templates select="../../pricingGroupLevelGroup[numberOfPax][$pos + 1]/fareInfoGroup" mode="totalbase">
	...
	<xsl:template match="fareInfoGroup" mode="totalprice">
		...
		<xsl:choose>
			<xsl:when test="($pos &lt; $loop) and ../../pricingGroupLevelGroup[numberOfPax][$pos + 1]/fareInfoGroup">
				<xsl:apply-templates select="../../pricingGroupLevelGroup[numberOfPax][$pos + 1]/fareInfoGroup" mode="totalprice">
					<xsl:with-param name="sum">
						<xsl:value-of select="$tot + $sum"/>
					</xsl:with-param>
					<xsl:with-param name="loop">
	...
	<xsl:template match="segmentLevelGroup" mode="fareinfo">
		...
		<FareInfo>
			<xsl:if test="$bp!=''">
				<xsl:attribute name="BreakPoint"><xsl:value-of select="substring($bp,$pos,1)"/></xsl:attribute>
			</xsl:if>
