...
<xsl:template match="/">
	<xsl:apply-templates select="Ticket_UpdateTSTReply"/>
...
<xsl:template match="Ticket_UpdateTSTReply">
	<OTA_StoredFareUpdateRS>
		<xsl:choose>
			...
			<xsl:when test="applicationError">
				<Errors>
					<xsl:apply-templates select="applicationError" mode="error"/>
				</Errors>
			</xsl:when>
		</xsl:choose>			
	</OTA_StoredFareUpdateRS>		
