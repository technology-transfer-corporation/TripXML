...
<xsl:template match="/">
	<OTA_QueueRS Version="1.000">
		...
		<xsl:apply-templates select="Queue_PlacePNRReply[recordLocator]"/>
		<xsl:apply-templates select="Queue_PlacePNRReply/errorReturn"/>
...
<!-- Queue List -->
<xsl:template match="Queue_ListReply">
	<ListQueue>
...
<!-- Queue Place -->
<xsl:template match="Queue_PlacePNRReply">
	<PlaceQueue>
		<UniqueID Type="16">
