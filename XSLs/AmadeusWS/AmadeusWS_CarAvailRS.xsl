...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ttVB="urn:ttVB" exclude-result-prefixes="msxsl ttVB" version="1.0">
	...
	<xsl:template match="/">
		<OTA_VehAvailRateRS Version="1.001">
			<xsl:apply-templates select="Car_SingleAvailabilityReply" />
			<xsl:apply-templates select="Car_MultiAvailabilityReply" />
	...
	<xsl:template match="Car_SingleAvailabilityReply">
		<xsl:choose>
	...
	<xsl:template match="Car_MultiAvailabilityReply">
		<xsl:choose>
