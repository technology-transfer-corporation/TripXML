...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/">
		<OTA_ShowMileageRS Version="1.001">
			<Success />
			<xsl:apply-templates select="Command_CrypticReply/longTextString/textStringDetails" />
		</OTA_ShowMileageRS>
	</xsl:template>
	<xsl:template match="textStringDetails">
		<xsl:variable name="screen2">
			<xsl:value-of select="substring(translate(string(.),'&#x0D;','/'),3)" />
		</xsl:variable>
		<xsl:variable name="screen1">
			<xsl:value-of select="substring-after($screen2,'XTRA/')" />
		</xsl:variable>
		<xsl:variable name="screen">
			<xsl:value-of select="substring-before($screen1,'PAGE ')" />
		</xsl:variable>
		<FromCity>
