...
<!-- ================================================================== -->
	...
	<xsl:template match="SabreCommandLLSRS" mode="error">
		<xsl:if test="not(contains(Response,'OK ')) and not(contains(Response,'ETR MESSAGE PROCESSED'))">
			<Error>
				<xsl:attribute name="Type">Sabre</xsl:attribute>
				<!--xsl:choose>
	...
	<xsl:template name="GetFlights">
		...
		<xsl:if test="contains($resp,' UC') or contains($resp,' LL') or contains($resp,' WL') or contains($resp,' HL')">
			...
			<xsl:element name="Error">
				...
				<xsl:variable name="day"><xsl:value-of select="substring($flight,9,2)"/></xsl:variable>
				...
				<xsl:choose>
					<xsl:when test="$month &lt; $cmonth"><xsl:value-of select="$cyear + 1"/></xsl:when>
					<xsl:when test="$month=$cmonth and $day &lt; $cday"><xsl:value-of select="$cyear + 1"/></xsl:when>
					<xsl:otherwise><xsl:value-of select="$cyear"/></xsl:otherwise>
				</xsl:choose>
				<xsl:text>-</xsl:text>
			...
			<xsl:call-template name="GetFlights">
				<xsl:with-param name="resp"><xsl:value-of select="substring-after($resp,$stCode)"/></xsl:with-param>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
