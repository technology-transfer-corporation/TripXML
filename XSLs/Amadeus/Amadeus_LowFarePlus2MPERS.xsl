...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	<!-- ================================================================== -->
	<!-- Amadeus_LowFarePlus2MPERS.xsl 												-->
	<!-- ================================================================== -->
	<!-- Date: 10 Jun 2009 - Rastko														-->
	<!-- ================================================================== -->
	...
	<xsl:template match="/">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:apply-templates select="PoweredLowestFareMPE_SearchReply" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="PoweredLowestFareMPE_SearchReply">
		<OTA_AirLowFareSearchRS>
		...
		<xsl:variable name="fm">
			<xsl:for-each select="segmentFlightRef/OD">
				...
				<xsl:for-each select="flightDetails">
					...
					<xsl:variable name="class"><xsl:value-of select="../../../paxFareProduct[1]/fareDetails[position()=$odpos]/groupOfFares/productInformation[position()=$posit]/cabinProduct/rbd"/></xsl:variable>
		...
		<FlightSegment>
			...
			<xsl:variable name="classes">
				<xsl:for-each select="../../../paxFareProduct[1]/fareDetails">
					<xsl:for-each select="groupOfFares/productInformation">
						<xsl:value-of select="cabinProduct/rbd" />
					</xsl:for-each>
				</xsl:for-each>
			</xsl:variable>
			...
			<TPA_Extensions>
				<xsl:variable name="cabin">
					<xsl:value-of select="../../../paxFareProduct[1]/fareDetails[position()=$odpos]/groupOfFares/productInformation[position()=$posit]/cabinProduct/cabin" />
				</xsl:variable>
				<CabinType>
				...
				<xsl:if test="../../../specificRecDetails[1]/specificProductDetails/fareContextDetails[position()=$odpos]/cnxContextDetails[position()=$posit]/fareCnxInfo/contextDetails/availabilityCnxType!=''">
					<FlightContext>
						<xsl:value-of select="../../../specificRecDetails[1]/specificProductDetails/fareContextDetails[position()=$odpos]/cnxContextDetails[position()=$posit]/fareCnxInfo/contextDetails/availabilityCnxType"/>
					</FlightContext>
				</xsl:if>
			</TPA_Extensions>
		</FlightSegment>
