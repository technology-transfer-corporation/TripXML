...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
			...
			<AirItinerary>
				<OriginDestinationOptions>
					<xsl:variable name="nip">
						<xsl:value-of select="count(paxFareProduct/paxReference[ptc != 'INF']/traveller)" />
					</xsl:variable>
	...
	<xsl:template match="referencingDetail">
		...
		<xsl:call-template name="seg">
			...
			<xsl:with-param name="sfr">1</xsl:with-param>
			<xsl:with-param name="refs"><xsl:value-of select="$ref" />/</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	<xsl:template name="seg">
		...
		<xsl:param name="sfr" />
		<xsl:param name="refs" />
		<xsl:apply-templates select="../../../flightIndex[position()=$pos]/groupOfFlights[propFlightGrDetail/flightProposal[1]/ref = $ref]/flightDetails">
			<xsl:with-param name="classes">
				<xsl:value-of select="$classes" />
			</xsl:with-param>
			<xsl:with-param name="nip">
			...
			<xsl:with-param name="rph">
				<xsl:value-of select="$sfr" />
			</xsl:with-param>
		</xsl:apply-templates>
		<xsl:for-each select="../../segmentFlightRef[position() > $sfr]">
			<xsl:variable name="possfr">
				<xsl:value-of select="position() + $sfr" />
			</xsl:variable>
			<xsl:variable name="ref1">
				<xsl:value-of select="referencingDetail[position()=$pos]/refNumber" />
			</xsl:variable>
			<xsl:if test="not(contains($refs,$ref1))">
				<xsl:call-template name="seg">
					<xsl:with-param name="classes">
						<xsl:value-of select="$classes" />
					</xsl:with-param>
					...
					<xsl:with-param name="sfr">
						<xsl:value-of select="$sfr + 1" />
					</xsl:with-param>
					<xsl:with-param name="refs"><xsl:value-of select="$refs" /><xsl:value-of select="$ref1" />/</xsl:with-param>
				</xsl:call-template>
			</xsl:if>
		</xsl:for-each>
	</xsl:template>
	<xsl:template match="flightDetails">
		...
		<FlightSegment>
			...
			<xsl:attribute name="ResBookDesigCode">
				<xsl:value-of select="substring($classes,$pos,1)" />
			</xsl:attribute>
