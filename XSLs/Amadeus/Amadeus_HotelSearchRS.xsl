...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/">
		<OTA_HotelSearchRS>
			...
			<xsl:if test="MessagesOnly_Reply/CAPI_Messages[LineType='EC'] or MessagesOnly_Reply/CAPI_Messages[LineType='E'] ">
				<Errors>
					<xsl:apply-templates select="MessagesOnly_Reply/CAPI_Messages" />
				</Errors>
			</xsl:if>
			<xsl:apply-templates select="PoweredHotel_ListReply" />
		</OTA_HotelSearchRS>
	</xsl:template>
	<!-- *********************************************************************************************************  -->
	<xsl:template match="PoweredHotel_ListReply">
		<Success />
		<Properties>
			...
			<xsl:if test="hotelProductInfo/pointOfReferenceDetails/porLatitude != ''">
				<Position>
					...
					<xsl:attribute name="Longitude">
						<xsl:value-of select="hotelProductInfopointOfReferenceDetails/porLongitude" />
					</xsl:attribute>
				</Position>
			</xsl:if>
