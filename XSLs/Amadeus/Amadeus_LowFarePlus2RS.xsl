...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	<!-- ================================================================== -->
	<!-- Amadeus_LowFarePlus2RS.xsl 													-->
	<!-- ================================================================== -->
	<!-- Date: 28 Nov 2012 - Rastko - removed black list							 		-->
	...
	<xsl:template match="recommendation">
		<PricedItinerary>
			...
			<AirItineraryPricingInfo>
				...
				<ItinTotalFare>
					<xsl:choose>
						<xsl:when test="paxFareProduct/fareDetails/productInformation[1]/fareProductDetail/fareType='RN'">
							<xsl:attribute name="NegotiatedFareCode" />
						</xsl:when>
						<xsl:when test="paxFareProduct/fareDetails/productInformation[1]/fareProductDetail/fareType='RA'">
			...
			<xsl:if test="segmentFlightRef/OD/flightDetails/flightInformation/addProductDetail/electronicTicketing != '' or paxFareProduct/fare/pricingMessage/freeTextQualification/messageQualifier = 		'LTD'">
				<TicketingInfo>
					<xsl:if test="paxFareProduct/fare/pricingMessage/freeTextQualification/messageQualifier = 'LTD'">
	...
	<xsl:template match="fareDetails" mode="FareRule">
		<xsl:variable name="ref"><xsl:value-of select="segmentRef/segmentRef"/></xsl:variable>
		<xsl:apply-templates select="productInformation" mode="FareRule">
	...
	<xsl:template match="paxFareProduct">
		<PTC_FareBreakdown>
			...
			<xsl:choose>
				<xsl:when test="fareDetails/productInformation/fareProductDetail/fareType='RP'">
					<xsl:attribute name="PricingSource">Published</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="PricingSource">Private</xsl:attribute>
			...
			<TPA_Extensions>
				<PricedCode>
					<xsl:choose>
						<xsl:when test="fareDetails[1]/productInformation/fareProductDetail/passengerType = 'CH'">CHD</xsl:when>
						<xsl:when test="fareDetails[1]/productInformation/fareProductDetail/passengerType = 'CNN'">CHD</xsl:when>
						<xsl:when test="fareDetails[1]/productInformation/fareProductDetail/passengerType = 'YCD'">SRC</xsl:when>
						<xsl:otherwise><xsl:value-of select="fareDetails[1]/productInformation/fareProductDetail/passengerType"/></xsl:otherwise>
					</xsl:choose>
				</PricedCode>
				<xsl:if test="fare/pricingMessage/freeTextQualification/messageQualifier = 'APM'">
					<Text><xsl:value-of select="fare/pricingMessage[freeTextQualification/messageQualifier='APM']/description"/></Text>
				</xsl:if>
			</TPA_Extensions>
		</PTC_FareBreakdown>
	</xsl:template>
	...
	<xsl:template match="flightDetails">
		...
		<FlightSegment>
			...
			<xsl:attribute name="ResBookDesigCode">
				<xsl:value-of select="../../../paxFareProduct[1]/fareDetails[position()=$odpos]/productInformation[position()=$posit]/avlProductDetails/rbd" />
				<!--xsl:value-of select="substring($classes,position(),1)" /-->
			</xsl:attribute>
			<xsl:attribute name="NumberInParty"><xsl:value-of select="count(../../../paxFareProduct/paxReference[ptc != 'INF' and ptc != 'IN' and ptc != 'ITF']/traveller)" /></xsl:attribute>
			...
			<TPA_Extensions>
				<xsl:variable name="cabin">
					<xsl:value-of select="../../../paxFareProduct[1]/fareDetails[position()=$odpos]/productInformation[position()=$posit]/avlProductDetails/cabin" />
				</xsl:variable>
				<CabinType>
	...
	<xsl:template match="productInformation" mode="FareRule">
		...
		<FareInfo>
			...
			<RuleInfo>
				<xsl:apply-templates select="../../fare/pricingMessage[freeTextQualification/messageQualifier = 'LTD']" mode="Advance"/> 
				<xsl:apply-templates select="../../fare/pricingMessage[freeTextQualification/messageQualifier = 'PEN']" mode="Penalty"/>
			</RuleInfo>
			<FilingAirline>
				<xsl:attribute name="Code">
	...
	<xsl:template match="pricingMessage" mode="Penalty">
		<xsl:choose>
			<xsl:when test="freeTextQualification/messageCode = '70' or freeTextQualification/messageCode = '71'">
				<ChargesRules>
					<VoluntaryChanges>
						<Penalty>									 
			...
			<xsl:when test="freeTextQualification/messageCode = '73'">
				<ChargesRules>
					<VoluntaryChanges>
						<Penalty>									 
	...
	<xsl:template match = "pricingMessage" mode="Advance">
		<xsl:choose>
			<xsl:when test="freeTextQualification/messageCode = '40'">
				<!--ResTicketingRules>
					<AdvResTicketing>
						<AdvTicketing>
