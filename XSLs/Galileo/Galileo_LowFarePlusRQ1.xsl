...
<!-- ================================================================== -->
<!-- Date: 24 Nov 2012 - Shashin - Changed CHD code mapping		-->
<!-- Date: 07 Aug - Rastko - corrected request to match low fare plus mapping		-->
	...
	<xsl:template name="create_each_pax_type">
		...
		<xsl:if test="$typeQ != 0">
			<Psgr>
              ...
              <!--given below line was changed in local xsl.Commented line show s the code given in local xsl-->
        ...
        <!--below given choose block was not there in teh local xsl-->
				<xsl:choose>
					<xsl:when test="@Code='CHD'"><Age>10</Age></xsl:when>
					<xsl:otherwise><Age><![CDATA[     ]]></Age></xsl:otherwise>
				</xsl:choose>
				<!--=======================================================-->
		...
		<AirAvailMods>
			...
			<GenAvail>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
            <!--slight value changes in local xsl-->
            <!--StartTmWnd>0000</StartTmWnd-->
						<StartTmWnd>0001</StartTmWnd>
					</xsl:otherwise>
				</xsl:choose>
				<xsl:choose>
					...
					<xsl:otherwise>
            <!--slight value changes in local xsl-->
            <!--StartTmWnd>2400</StartTmWnd-->
						<EndTmWnd>2359</EndTmWnd>
					</xsl:otherwise>
				</xsl:choose>
				<!-- below given code block was not there in thelocal xsl -->
        <JrnyTm/>
				<FltTypeInd/>
				<FltTypePref/>
				<StartPtInd>B</StartPtInd>
				<EndPtInd>B</EndPtInd>
				...
				<EndTmWnd>
					...
					<xsl:choose>
						<xsl:when test="$end &lt;= 2400">
							<xsl:value-of select="format-number($end,'0000')"/>
						</xsl:when>
