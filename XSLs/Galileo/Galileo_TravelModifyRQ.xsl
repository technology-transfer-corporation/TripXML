...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	<!-- ================================================================== -->
	<!-- Galileo_TravelModifyRQ.xsl 														-->
	<!-- ================================================================== -->
	<!-- Date: 20 Mar 2011 - Rastko - made changes to support adding air segment and pricing	-->
	<!-- Date: 03 Mar 2009 - Rastko														-->
	<!-- ================================================================== -->
	<xsl:output method="xml" omit-xml-declaration="yes" />
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="OTA_TravelModifyRQ/@ResStatus='Modify'">
				<xsl:apply-templates select="OTA_TravelModifyRQ" mode="modify" />
			</xsl:when>
			<xsl:otherwise>
				<PNRBFManagement_17>
					<xsl:apply-templates select="OTA_TravelModifyRQ" mode="start" />
				</PNRBFManagement_17>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<xsl:template match="OTA_TravelModifyRQ" mode="start">
		<PNRBFRetrieveMods>
			<PNRAddr>
				<FileAddr /> 
			  	<CodeCheck /> 
			  	<RecLoc><xsl:value-of select="UniqueId/@ID"/></RecLoc> 
			</PNRAddr>
		</PNRBFRetrieveMods>
		<xsl:apply-templates select="TPA_Extensions/PNRData"/>
		<xsl:if test="OTA_AirBookModifyRQ/AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment">
			<AirSegSellMods>
				<xsl:choose>
					<xsl:when test="OTA_AirBookModifyRQ/AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment/@ActionCode = 'Other'">
						<xsl:apply-templates select="OTA_AirBookModifyRQ/AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption" mode="Passive" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:apply-templates select="OTA_AirBookModifyRQ/AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption" mode="Regular" />
					</xsl:otherwise>
				</xsl:choose>
			</AirSegSellMods>
			<FareQuoteVerifyMods>
		...
		<xsl:if test="OTA_AirBookModifyRQ/AirBookModifyRQ/TravelerInfo/SpecialReqDetails or OTA_AirBookModifyRQ/Fulfillment/PaymentDetails/PaymentDetail or OTA_AirBookModifyRQ/Fulfillment/DeliveryAddress or TPA_Extensions/PNRData/AccountingLine or TPA_Extensions/PNRData/Traveler/@PassengerTypeCode = 'CHD' or TPA_Extensions/AgencyData/ServiceFee/@Amount !=''">
			<PNRBFSecondaryBldChgMods>
				<ItemAry>
					<!-- create CHD OSI -->
					<xsl:apply-templates select="TPA_Extensions/PNRData/Traveler[@PassengerTypeCode='CHD']" mode="osi" />
					<!-- SSR, OSI, Remark, Special Remark -->
					<xsl:apply-templates select="OTA_AirBookModifyRQ/AirBookModifyRQ/TravelerInfo/SpecialReqDetails"/>
					<!--  FOP Qual       -->
					<xsl:apply-templates select="OTA_AirBookModifyRQ/Fulfillment/PaymentDetails/PaymentDetail" />
					<!-- Delivery Address Qual  -->
					<xsl:apply-templates select="OTA_AirBookModifyRQ/Fulfillment/DeliveryAddress"/>
					<!-- Email -->
		...
		<xsl:if test="OTA_AirBookModifyRQ/AirBookModifyRQ/TravelerInfo/SpecialReqDetails/SeatRequests">
			<SeatSellMods>
				<ReqAry>
					<xsl:apply-templates select="OTA_AirBookModifyRQ/AirBookModifyRQ/TravelerInfo/SpecialReqDetails/SeatRequests" />
				</ReqAry>
			</SeatSellMods>
		</xsl:if>
		...
		<xsl:call-template name="EndTransactionMods"/>
	</xsl:template>
	...
	<xsl:template match="HotelReservation">
		<xsl:choose>
			<xsl:when test="../../../OTA_HotelResRQ/@ResStatus='OnRequest'">
				<PassiveSegmentSellMods>
					<PassiveSegmentSellRequest>
					  <PassiveSegType>HTL</PassiveSegType> 
					  <Vnd><xsl:value-of select="RoomStays/RoomStay/BasicPropertyInfo/@ChainCode" /></Vnd> 
					  <Status>BK</Status> 
					  <NumItems>01</NumItems> 
					  <City><xsl:value-of select="RoomStays/RoomStay/BasicPropertyInfo/@HotelCityCode"/></City> 
					  <StartDt><xsl:value-of select="substring(translate(string(RoomStays/RoomStay/TimeSpan/@Start),'-',''),1,8)" /></StartDt> 
					  <EndDt><xsl:value-of select="substring(translate(string(RoomStays/RoomStay/TimeSpan/@End),'-',''),1,8)" /></EndDt>
					  <DuePaidTextInd></DuePaidTextInd> 
					  <AmtDuePaid></AmtDuePaid> 
				  	</PassiveSegmentSellRequest>
				  	<PassiveSegmentSellFreeformRequest>
						<Text><xsl:value-of select="concat('PC-',RoomStays/RoomStay/BasicPropertyInfo/@HotelCode)"/></Text>
					</PassiveSegmentSellFreeformRequest>
				</PassiveSegmentSellMods>
			</xsl:when>
			<xsl:otherwise>
		...
		<PNRBFPrimaryBldChgMods>
			<ItemAry>
				...
				<xsl:choose>
					...
					<xsl:otherwise><xsl:apply-templates select="../../OTA_AirBookModifyRQ/Ticketing" /></xsl:otherwise>
				</xsl:choose>
				<Item>
	...
	<xsl:template match="SpecialReqDetails">
		...
		<!-- AssocRmkQual  -->
		<xsl:apply-templates select="SpecialRemarks/SpecialRemark[@RemarkType='Air']"
			mode="Air" />
		<xsl:choose>
			<xsl:when test="//POS/TPA_Extensions/Provider/Name='Galileo'">
				<xsl:apply-templates select="SpecialRemarks/SpecialRemark[@RemarkType='Invoice']"
					mode="Galileo" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="SpecialRemarks/SpecialRemark[@RemarkType='Invoice']"
					...
					<LNameRmk>
						<xsl:if test="../../../POS/TPA_Extensions/Provider/Name = 'Apollo'">
							<xsl:choose>
								...
								<xsl:when test="@PassengerTypeCode='CHD' and Age != ''">C<xsl:value-of select="Age" /></xsl:when>
								...
								<xsl:when test="@PassengerTypeCode='INF'">INF</xsl:when>
	...
	<xsl:template match="Traveler" mode="osi">
		<Item>
			...
			<OSIQual>
				...
				<AddQual>
					<OSIV>
						<!--xsl:value-of select="../../../OTA_AirBookModifyRQ/AirItinerary/OriginDestinationOptions[1]/OriginDestinationOption[1]/FlightSegment[1]/MarketingAirline/@Code" />
						<xsl:text><![CDATA[ ]]></xsl:text-->
						<xsl:text>YY</xsl:text>
					</OSIV>
					<OSI>
			...
			<DOCInvoiceQual>
				...
				<AddQual>
					...
					<Text>
						...
						<xsl:choose>
							<xsl:when test="../../../OTA_AirBookModifyRQ/Fulfillment/PaymentDetails/PaymentDetail/DirectBill/@DirectBill_ID='Cash'">-CASH</xsl:when>
	...
	<xsl:template match="PriceData">
		<StorePriceMods>
			...
			<xsl:if test="../../OTA_AirBookModifyRQ/Ticketing/@TicketType = 'Paper'">
				<DocumentSelect>
					<ETInd>P</ETInd> 
					<!--ItinInd>I</ItinInd--> 
				</DocumentSelect>
			...
			<xsl:if test="../../../POS/TPA_Extensions/Provider/Name = 'Galileo'">
				<PlatingAirVMods>
					<PlatingAirV><xsl:value-of 	select="../../OTA_AirBookModifyRQ/AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment/MarketingAirline/@Code"/> </PlatingAirV>
				</PlatingAirVMods>
			</xsl:if>
			<PsgrMods>
				<PsgrAry>
			...
			<xsl:if test="../../POS/TPA_Extensions/Provider/Name = 'Galileo'">
				<PlatingAirVMod>
					<AirV>
						<xsl:choose>
							<xsl:when test="@ValidatingAirlineCode != ''">
								<xsl:value-of select="@ValidatingAirlineCode"/>
							</xsl:when>
							<xsl:when test="@FlightRefNumberRPHList != ''">
								<xsl:variable name="airseg">
									<xsl:choose>
										<xsl:when test="contains(@FlightRefNumberRPHList,' ')">
											<xsl:value-of select="substring-before(@FlightRefNumberRPHList,' ')"/>
										</xsl:when>
										<xsl:otherwise><xsl:value-of select="@FlightRefNumberRPHList"/></xsl:otherwise>
									</xsl:choose>
								</xsl:variable>
								<xsl:value-of 	select="../../OTA_AirBookModifyRQ/AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment[@RPH=$airseg]/MarketingAirline/@Code"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of 	select="../../OTA_AirBookModifyRQ/AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment/MarketingAirline/@Code"/>
							</xsl:otherwise>
						</xsl:choose>
					</AirV>
				</PlatingAirVMod>
	...
	<xsl:template match="OTA_TravelItineraryRQ" mode="et">
		<PNRBFEnd_7_0>
			<EndTransactionMods>
				<xsl:choose>
					<xsl:when test="OTA_AirBookModifyRQ/Queue">
						<TypeInd>Q</TypeInd>
						<RcvdFrom>
						...
						<ChgQEPQual>
							<PCC>
								<xsl:value-of select="OTA_AirBookModifyRQ/Queue/@PseudoCityCode" />
							</PCC>
							<PCCQNum>
								<xsl:value-of select="OTA_AirBookModifyRQ/Queue/@QueueNumber" />
							</PCCQNum>
							<xsl:if test="OTA_AirBookModifyRQ/Queue/@QueueCategory">
								<QCat>
									<xsl:value-of select="OTA_AirBookModifyRQ/Queue/@QueueCategory" />
								</QCat>
							</xsl:if>
						</ChgQEPQual>
					</xsl:when>
	...
	<xsl:template match="OTA_TravelModifyRQ" mode="modify">
		<ModifyRQ>
			<Retrieve>
				<PNRBFManagement_13>
					<PNRBFRetrieveMods>
			...
			<Modify>
				<PNRBFManagement_13>
					<xsl:apply-templates select="OTA_AirBookModifyRQ" mode="modify" />
				</PNRBFManagement_13>
			</Modify>
			<VerifyATFQ>
				<PNRBFManagement_13>
	...
	<xsl:template match="OTA_AirBookModifyRQ" mode="modify">
		<xsl:if test="AirBookModifyRQ/AirItinerary or AirReservation/AirItinerary">
			<DateClassMods>
				<DateClassInfo>
					<NewDate>
						<xsl:value-of select="translate(substring(AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment/@DepartureDateTime,1,10),'-','')"/>
					</NewDate>
					<SeqNumAry>
						<SeqNumItem>
							<SeqNum>01</SeqNum>
							<NewClass>
								<xsl:choose>
									<xsl:when test="AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment/@ResBookDesigCode = AirReservation/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment/@ResBookDesigCode">
										<xsl:text>99</xsl:text>
									</xsl:when>
									<xsl:otherwise>
										<xsl:text> </xsl:text>
										<xsl:value-of select="AirBookModifyRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment/@ResBookDesigCode"/>
									</xsl:otherwise>
								</xsl:choose>
							</NewClass>
						</SeqNumItem>
	...
	<xsl:template name="EndTransactionMods">
		<EndTransactionMods>
			<EndTransactRequest>
				<ETInd>
					<xsl:choose>
