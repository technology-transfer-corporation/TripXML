...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ttVB="urn:ttVB" version="1.0">
	...
	<!-- Date: 03 Nov 2009 - Rastko - added ArrivalDateTime calculation				-->
	...
	<xsl:template match="PNRBFManagement_17 | PNRBFManagement_53">
		<xsl:choose>
			...
			<xsl:otherwise>
				<Success />
				<xsl:if test="(DocProdDisplayStoredQuote/ErrText/Text != '' and not(DocProdDisplayStoredQuote/FareNumInfo)) or PNRBFPrimaryBldChg/RecID = 'EROR' or PNRBFSecondaryBldChg/RecID = 'EROR' or DocProdFareManipulation_4_0/TransactionErrorCode or FareInfo/RespHeader/ErrMsg = 'Y'">
					<Warnings>
						<xsl:choose>
				...
				<TravelItinerary>
					<ItineraryRef>
						...
						<xsl:attribute name="ID_Context"><xsl:value-of select="PNRBFRetrieve/GenPNRInfo/OwningAgncyPCC"/></xsl:attribute>
					</ItineraryRef>
	...
	<xsl:template match="PNRBFRetrieve">
		...
		<ItineraryInfo>
			...
			<!--******************************************************-->
			<xsl:if test="SeatSeg or ProgramaticSSR or NonProgramaticSSR or OSI or GenRmkInfo or  ItinRmk or InvoiceRmk[Keyword != '3007']">
				<SpecialRequestDetails>
					...
					<xsl:if test="ItinRmk or InvoiceRmk[Keyword != '3007']">
						<SpecialRemarks>
							<xsl:if test="ItinRmk">
		...
		<xsl:if test="Percent != '' or Amt != ''">
			<AgencyCommission>
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:attribute name="Amount"><xsl:value-of select="Amt"/></xsl:attribute>
					</xsl:otherwise>
				</xsl:choose>
			</AgencyCommission>
		</xsl:if>
	...
	<xsl:template match="DocProdDisplayStoredQuote">
		<ItemPricing>
			...
			<xsl:if test="AssocPsgrs/PsgrAry/Psgr/AbsNameNum != ''">
	...
	<xsl:template match="GenQuoteDetails" mode="TotalTax">
		<xsl:for-each select="TaxDataAry/TaxData">
			...
			<xsl:variable name="subtract"><xsl:value-of select="concat(1,substring($addzeros,1,$numberdecimals))" /></xsl:variable>
	...
	<xsl:template match="AirSeg | OpenAirSeg" mode="Segment">
		<Item>
            ...
            <!--below given code line was changed in local xsl. commented line gives teh local xsl's codeline-->
            ...
            <xsl:attribute name="ArrivalDateTime">
                    ...
                    <!--xsl:when test="count(../VndRecLocs/RecLocInfoAry) = 1"-->
                    <xsl:value-of select="../VndRecLocs/RecLocInfoAry/RecLocInfo/RecLoc"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:variable name="air"><xsl:value-of select="AirV"/></xsl:variable>
										
                    <!--below given choose block was not there in local xsl-->
                    <xsl:choose>
	...
	<xsl:template match="AirSegOpAirV">
		...
		<xsl:template match="ARNK" mode="Segment">
			<Item>
				<TPA_Extensions><Arnk/></TPA_Extensions>
			</Item>
		</xsl:template>
	<!--************************************************************************************-->
	...
	<xsl:template match="Email">
		<Email><xsl:value-of select="translate(Data,'¤','@')"/></Email>
	</xsl:template>
	<!--************************************************************************************-->
	...
	<xsl:template match="Addr | DeliveryAddr">
		<xsl:variable name="addr"><xsl:value-of select="translate(string(.),'¤','@')"/></xsl:variable>
		<xsl:variable name="FieldName" select="substring-before($addr,'@')" /> <!--name -->
		<xsl:variable name="Field2Temp" select="substring-after($addr,'@')" />
		<xsl:variable name="FieldStreet" select="substring-before($Field2Temp,'@')" /> <!--street -->
		<xsl:variable name="Field3Temp" select="substring-after($Field2Temp,'@')" />
		<xsl:variable name="FieldCity" select="substring-before($Field3Temp,'@')" /> <!--City -->
		<xsl:variable name="Field4Temp" select="substring-after($Field3Temp,'@')" /> 
		<xsl:variable name="FieldStzip" select="substring-before($Field4Temp,'@')" /> <!--State and zip -->
		<xsl:variable name="FieldCountry" select="substring-after($Field4Temp,'@')" /> <!--Country -->
			<StreetNmbr>
			...
			<PostalCode>
				<xsl:choose>
					...
					<xsl:otherwise><xsl:value-of select="substring-after($FieldStzip,' Z/')" /></xsl:otherwise>
				</xsl:choose>
			</PostalCode>
				...
				<StateProv>
					<xsl:choose>
						<xsl:when test="substring($FieldStzip,3,1) = ' '">
							<xsl:attribute name="StateCode"><xsl:value-of select="substring-before($FieldStzip,' ')"/></xsl:attribute>
						</xsl:when>
						<xsl:otherwise><xsl:value-of select="substring-before($FieldStzip,' ')"/></xsl:otherwise>
					</xsl:choose>
				</StateProv>
			</xsl:if>
			<CountryName>
		...
		<PaymentCard>
			<xsl:attribute name="CardCode">
				<xsl:choose>
					...
					<xsl:otherwise><xsl:value-of select="Vnd" /></xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
	...
	<xsl:template match="SeatSeg">
		<xsl:variable name="FltSegNum"><xsl:value-of select="FltSegNum"/></xsl:variable>
		<xsl:variable name="StartAirp"><xsl:value-of select="StartAirp"/></xsl:variable>
		<xsl:variable name="EndAirp"><xsl:value-of select="EndAirp"/></xsl:variable>
		<xsl:apply-templates select="following-sibling::SeatAssignment[1]">
			<xsl:with-param name="FltSegNum"><xsl:value-of select="$FltSegNum"/></xsl:with-param>
			<xsl:with-param name="StartAirp"><xsl:value-of select="$StartAirp"/></xsl:with-param>
			<xsl:with-param name="EndAirp"><xsl:value-of select="$EndAirp"/></xsl:with-param>
		</xsl:apply-templates>
	</xsl:template>
		...
		<SeatRequest>
			...
			<xsl:choose>
				...
				<xsl:when test="AttribAry/Attrib!=''">
					<xsl:attribute name="SeatPreference"><xsl:value-of select="AttribAry/Attrib"/></xsl:attribute>
				</xsl:when>
			</xsl:choose>
			...
			<DepartureAirport>
				<xsl:attribute name="LocationCode"><xsl:value-of select="$StartAirp"/></xsl:attribute>
			</DepartureAirport>
			<ArrivalAirport>
				<xsl:attribute name="LocationCode"><xsl:value-of select="$EndAirp"/></xsl:attribute>
			</ArrivalAirport>
		</SeatRequest>
	...
	<xsl:template match="ProgramaticSSR">
		<SpecialServiceRequest>
			...
			<xsl:if test="../AirSeg[SegNum=current()/SegNum]/AirV">
				<Airline>
					<xsl:attribute name="Code"><xsl:value-of select="../AirSeg[SegNum=current()/SegNum]/AirV" /></xsl:attribute>
					<xsl:attribute name="CodeContext"><xsl:value-of select="Status"/></xsl:attribute>
				</Airline>
			</xsl:if>
	...
	<xsl:template match="NonProgramaticSSR">
		<SpecialServiceRequest>
			...
			<xsl:if test="Vnd!=''">
				<Airline>
					<xsl:attribute name="Code"><xsl:value-of select="Vnd" /></xsl:attribute>
					<xsl:attribute name="CodeContext"><xsl:value-of select="Status"/></xsl:attribute>
				</Airline>
			</xsl:if>
			<Text>
	...
	<xsl:template match="OSI">
		<OtherServiceInformation>
			<Airline>
				<xsl:attribute name="Code"><xsl:value-of select="OSIV" /></xsl:attribute>
			</Airline>
			<Text>
		...
		<Remark>
			...
			<xsl:variable name="rmk1">
				<xsl:choose>
					...
					<xsl:otherwise><xsl:value-of select="GenRmk"/></xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
		...
		<xsl:if test="$PsgrsTot > 0">
			...
			<CustomerInfo>
					...
					<xsl:if test="../GenPNRInfo/OwningCRS = '1G'">
						<xsl:if test="../NameRmkInfo[LNameNum=$ItemNo]/NameRmk != ''">
							<xsl:if test="string-length(../NameRmkInfo[LNameNum=$ItemNo]/NameRmk) > 5">
								<!--teh below given if condition was not there in local xsl.But con tent was there-->
                ...
                <!--===============================================================================-->
					...
					<xsl:variable name="lnamenum"><xsl:value-of select="LNameNum"/></xsl:variable>
	...
	<xsl:template name="bdt">
		...
		<xsl:variable name="bd">
			<xsl:choose>
				<xsl:when test="string-length($bdt) > 7"><xsl:value-of select="substring($bdt,1,7)"/></xsl:when>
				<xsl:otherwise><xsl:value-of select="$bdt"/></xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:choose>
	...
	<xsl:template match="FreqCustInfo">
		<CustLoyalty>
			<xsl:attribute name="ProgramID"><xsl:value-of select="FreqCustV"/></xsl:attribute>
			<xsl:if test="FreqCustStatus != ''">
				<xsl:attribute name="LoyalLevel"><xsl:value-of select="FreqCustStatus"/></xsl:attribute>
			</xsl:if>
			<xsl:attribute name="MembershipID"><xsl:value-of select="FreqCustNum"/></xsl:attribute>
		</CustLoyalty>
	</xsl:template>
				...
				<Guarantee>
						...
						<GuaranteesAccepted>
							<GuaranteeAccepted>
								<xsl:variable name="Contents">
									<xsl:choose>
										<xsl:when test="substring(Contents,1,4)='DPST'"><xsl:value-of select="substring(Contents,5)"/></xsl:when>
										<xsl:otherwise><xsl:value-of select="substring(Contents,1)"/></xsl:otherwise>
									</xsl:choose>
								</xsl:variable>
								<PaymentCard>
									<xsl:attribute name="CardCode">
					...
					<xsl:if test="contains(Contents, 'AGT')">
						<GuaranteeDescription>
							<Text><xsl:value-of select="Contents"/></Text>
						</GuaranteeDescription>
					</xsl:if>
				</Guarantee>
			</xsl:when>
	...
	<xsl:template match="CarSeg">
		<Item>
			...
			<Vehicle>
						...
						<RateDistance>
							<xsl:choose>
								...
								<xsl:otherwise>
									...
									<xsl:attribute name="Quantity"><xsl:value-of select="MileAllow" /></xsl:attribute>
								</xsl:otherwise>
							</xsl:choose>
						...
						<xsl:if test="ExtraHourMileAllow !=''">
							<VehicleCharge>
								...
								<Calculation>
									...
									<xsl:attribute name="Quantity">
										<!--teh below given coose block was not given in local xsl file-->
                    <xsl:choose>
											<xsl:when test="ExtraHourMileAllow='UNL'">99999</xsl:when>
											<xsl:otherwise><xsl:value-of select="ExtraHourMileAllow" /></xsl:otherwise>
										</xsl:choose>
						...
						<xsl:if test="ExtraDayMileAllow!=''">
							<VehicleCharge>
								...
								<Calculation>
									...
									<xsl:attribute name="Quantity">
                    <!--******************************************************-->
                    <!--below given coose block was not there in the local xsl-->
										<xsl:choose>
											<xsl:when test="ExtraDayMileAllow='UNL'">99999</xsl:when>
											<xsl:otherwise><xsl:value-of select="ExtraDayMileAllow" /></xsl:otherwise>
										</xsl:choose>
                    <!--======================================================-->
									</xsl:attribute>
								</Calculation>
				...
				<xsl:if test="RateAmt != ''">
					<TotalCharge>
						<xsl:attribute name="RateTotalAmount"><xsl:value-of select="RateAmt" /></xsl:attribute>
						<xsl:attribute name="EstimatedTotalAmount"><xsl:value-of select="RateAmt" /></xsl:attribute>
						<xsl:attribute name="CurrencyCode">
							<xsl:choose>
								<xsl:when test="Currency!=''">
									<xsl:value-of select="Currency" />
				...
				<TPA_Extensions>
					<xsl:if test="../../CarSegSellText">
						<Rules>
							<xsl:for-each select="../../CarSegSellText/LineDescAry/LineDescInfo">
								<Text><xsl:value-of select="Txt"/></Text>
							</xsl:for-each>
						</Rules>
					</xsl:if>
					<CarOptions>
	...
	<xsl:template match="NonAirSeg">
		<Item>
			<xsl:attribute name="RPH"><xsl:value-of select="SegNum"/></xsl:attribute>
