...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <!-- ================================================================== -->
  <!-- Galileo_LowFarePlusRS1.xsl 														-->
  <!-- ================================================================== -->
  <!-- Date: 21 Jun 2013  - Suraj - Added Pricing Source to TPA_Extensions (Line 610)			-->
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="FareQuoteSuperBB_9"/>
    <xsl:apply-templates select="FareQuoteSuperBB_25"/>
  </xsl:template>
  <!-- ************************************************************** -->
  <xsl:template match="FareQuoteSuperBB_9|FareQuoteSuperBB_25">
      ...
      <AirItinerary>
        ...
        <OriginDestinationOptions>
          <xsl:variable name="nip">
            <xsl:value-of select="sum(PsgrTypes[PICReq != 'INF']/PICPsgrs)"/>
          </xsl:variable>
  ...
  <xsl:template match="FareInfo" mode="totalprice">
    ...
    <AirItineraryPricingInfo>
      ...
      <ItinTotalFare>
        <xsl:if test="../OTA_AirLowFareSearchPlusRQ/TravelerInfoSummary/PriceRequestInformation/NegotiatedFareCode/@SecondaryCode!=''">
          <xsl:attribute name="NegotiatedFareCode">
            <xsl:value-of select="../OTA_AirLowFareSearchPlusRQ/TravelerInfoSummary/PriceRequestInformation/NegotiatedFareCode/@SecondaryCode"/>
          </xsl:attribute>
        </xsl:if>
  ...
  <xsl:template match="GenQuoteDetails" mode="travelergroup">
    ...
    <PTC_FareBreakdown>
      ...
      <FareBasisCodes>
        <xsl:for-each select="../SegRelatedInfo[UniqueKey=number($paxno)]">
          <FareBasisCode>
        ...
        <Taxes>
          <Tax>
            ...
            <xsl:variable name="tta">
              <xsl:choose>
                <!--below given code line was changed in local xsl. it is given below here as a commented line -->
      ...
      <TPA_Extensions>
        <PricedCode>
          ...
          <xsl:choose>
            ...
            <xsl:when test="$PsgrTypeResp = 'CNN'">CHD</xsl:when>
              ...
              <VoluntaryChanges>
                <Penalty>
                  <xsl:attribute name="PenaltyType">
                    <xsl:value-of select="../InfoMsg[contains(Text,'CHANGE')]/Text"/>
                  </xsl:attribute>
                </Penalty>
              </VoluntaryChanges>
            </ChargesRules>
          ...
          <xsl:when test="../InfoMsg[contains(Text,'REFUND')]">
            <ChargesRules>
              <VoluntaryChanges>
                <Penalty>
                  <xsl:attribute name="PenaltyType">
                    <xsl:value-of select="../InfoMsg[contains(Text,'REFUND')]/Text"/>
                  </xsl:attribute>
                </Penalty>
              </VoluntaryChanges>
            </ChargesRules>
          ...
          <xsl:when test="../InfoMsg[contains(Text,'CHG')]">
            <ChargesRules>
              <VoluntaryChanges>
                <Penalty>
                  <xsl:attribute name="PenaltyType">
                    <xsl:value-of select="../InfoMsg[contains(Text,'CHG')]/Text"/>
                  </xsl:attribute>
                </Penalty>
              </VoluntaryChanges>
            </ChargesRules>
  ...
  <xsl:template match="PenFee" mode="pen">
    <Penalty>
      ...
      <xsl:attribute name="DepositRequired">
        <xsl:value-of select="DepRequired"/>
      </xsl:attribute>
  ...
  <xsl:template match="PenFee" mode="fee">
    <FeeApplies>
      <xsl:attribute name="FailureToConfirm">
        <xsl:value-of select="FailConfirmSpace"/>
      </xsl:attribute>
      <xsl:attribute name="Cancellation">
        <xsl:value-of select="Cancellation"/>
      </xsl:attribute>
      <xsl:attribute name="Carrier">
        <xsl:value-of select="AirVFee"/>
      </xsl:attribute>
      <xsl:attribute name="ReplaceTicket">
        <xsl:value-of select="ReplaceTk"/>
      </xsl:attribute>
      <xsl:attribute name="ChangeItinerary">
        <xsl:value-of select="ItinChg"/>
      </xsl:attribute>
    </FeeApplies>
