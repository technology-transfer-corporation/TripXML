...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <!-- ================================================================== -->
  <!-- v04_Galileo_TravelBuildRQ.xsl - v04												-->
  <!-- ================================================================== -->
  <!-- Date: 10 Sep 2014 - Rastko - added support for additonal options in car segment	-->
  ...
  <!-- Date: 05 Oct 2011 - Rastko - corrected mapping of optional fields in car sell group	-->
  <!-- Date: 01 Jun 2011 - Rastko - mapped validating carriers again					-->
  <!-- Date: 19 Apr 2011 - Rastko - made changes for Galileo certification			-->
  <!-- Date: 13 Apr 2011 - Rastko - corrected mapping of validating carrier			-->
  <!-- Date: 20 Mar 2011 - Rastko - added support for hotel passive segments		-->
  <!-- Date: 20 Feb 2011 - Rastko - added agency IATA number as guarantee		-->
  <!-- Date: 18 Feb 2011 - Rastko - corrected mapping of SSR DOCS				-->
  <!-- Date: 18 Jan 2011 - Rastko - corrected hotel optional elements mapping		-->
  <!-- Date: 24 Oct 2009 - Rastko - added BR purchase order option to car seg (direct bill)	-->
  <!-- Date: 18 Oct 2010 - Rastko - added passenger ID in car segment				-->
  <!-- Date: 21 Jul 2010 - Rastko - hard coded PCC process in Rules				-->
  <!-- Date: 05 Jun 2010 - Rastko - added support for price again when no valid fare found	-->
  <!-- Date: 14 Feb 2009 - Rastko - added support for due/paid/text					-->
  <!-- ================================================================== -->
  <xsl:output method="xml" omit-xml-declaration="yes" />
  <xsl:variable name="GDS">
    <xsl:value-of select="OTA_TravelItineraryRQ/POS/TPA_Extensions/Provider/Name"/>
  </xsl:variable>
  ...
  <xsl:template match="OTA_TravelItineraryRQ" mode="start">
    <xsl:if test="OTA_VehResRQ[not(VehResRQCore/@Status) or VehResRQCore/@Status!='OnRequest']">
      <CarAvail>
        <CarStandardAvail_6_0>
          <CarAvailMods>
            ...
            <CarV1>
              <xsl:value-of select="OTA_VehResRQ/VehResRQCore/VendorPref/@Code" />
            </CarV1>
            <CarType1>
    ...
    <xsl:if test="TPA_Extensions/PriceData[@FlightRefNumberRPHList!='']">
      <PriceAgain>
        <PNRBFManagement_17>
          <xsl:apply-templates select="TPA_Extensions/PriceData[@FlightRefNumberRPHList!='']" mode="priceagain"/>
        </PNRBFManagement_17>
      </PriceAgain>
    </xsl:if>
  </xsl:template>
      ...
      <xsl:when test="../../../OTA_HotelResRQ/@ResStatus='OnRequest'">
        <PassiveSegmentSellMods>
          <PassiveSegmentSellRequest>
            ...
            <StartDt>
              <xsl:value-of select="substring(translate(string(RoomStays/RoomStay/TimeSpan/@Start),'-',''),1,8)" />
            </StartDt>
            <EndDt>
              <xsl:value-of select="substring(translate(string(RoomStays/RoomStay/TimeSpan/@End),'-',''),1,8)" />
            </EndDt>
          ...
          <PassiveSegmentSellFreeformRequest>
            <Text>
              <xsl:value-of select="concat('PC-',RoomStays/RoomStay/BasicPropertyInfo/@HotelCode)"/>
            </Text>
          </PassiveSegmentSellFreeformRequest>
        </PassiveSegmentSellMods>
      </xsl:when>
  ...
  <!--	**		 	            Address                			                         **-->
  <!--*********************************************************************************-->
  <xsl:template match="Address" mode="hotel">
    <Fld>
      <ID>AD</ID>
      <Contents>
  ...
  <xsl:template match="Address" mode="car">
    <OptFldID>
      <ID>AD</ID>
      <Contents>
        <xsl:value-of select="StreetNmbr" />
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <CarSegSellMods>
          ...
          <!--	**			Process Optional Fields                 	                        **-->
          <!--*********************************************************************************-->
          <xsl:if test="VehResRQCore/RateQualifier/@CorpDiscountNmbr or VehResRQCore/RateQualifier/@PromotionCode or VehResRQInfo/RentalPaymentPref/PaymentCard or VehResRQInfo/@SmokingAllowed='0' or ../../OTA_TravelItineraryRQ/TPA_Extensions/PNRData/Traveler/Address or 	../../OTA_TravelItineraryRQ/TPA_Extensions/PNRData/Traveler/CustLoyalty or string-length(VehResRQCore/VehRentalCore/PickUpLocation/@LocationCode)>3  or VehResRQInfo/RentalPaymentPref/Voucher/@SeriesCode!='' or VehResRQInfo/RentalPaymentPref/DirectBill/@DirectBill_ID!=''">
            <OptFldIDAry>
              <xsl:if test="string-length(VehResRQCore/VehRentalCore/PickUpLocation/@LocationCode)>3">
                <OptFldID>
                  <ID>PU</ID>
                  <Contents>
                    <xsl:value-of select="VehResRQCore/VehRentalCore/PickUpLocation/@LocationCode"/>
              ...
              <!--	**			Process Address/CustLotalty	                   	            **-->
              <!--*********************************************************************************-->
              <xsl:apply-templates select="../../OTA_TravelItineraryRQ/TPA_Extensions/PNRData/Address" mode="car"/>
              <xsl:apply-templates select="../../OTA_TravelItineraryRQ/TPA_Extensions/PNRData/Traveler/CustLoyalty" mode="car" />
              <!--*********************************************************************************-->
              <!--	**			Process Booking Source                      	                  **-->
              <!--*********************************************************************************-->
              <xsl:if test="../../OTA_TravelItineraryRQ/POS/Source/RequestorID/@ID">
              ...
              <!--	**			Purchase order				                      	                  **-->
              <!--*********************************************************************************-->
              <xsl:if test="VehResRQInfo/RentalPaymentPref/DirectBill/@DirectBill_ID!=''">
                <OptFldID>
                  <ID>BR</ID>
                  <Contents>
                    <xsl:value-of select="VehResRQInfo/RentalPaymentPref/DirectBill/@DirectBill_ID" />
                  </Contents>
                </OptFldID>
              </xsl:if>
            </OptFldIDAry>
  ...
  <xsl:template match="UniqueRemark" mode="dpt">
    <Item>
      ...
      <DuePaidTextQual>
        ...
        <AddQual>
          <Type>
            <xsl:value-of select="concat('A',substring-before(.,'/'))"/>
          </Type>
          <InDt>
  ...
  <xsl:template match="PriceData">
    <StorePriceMods>
      ...
      <xsl:if test="../../POS/TPA_Extensions/Provider/Name = 'Galileo' and count(../PriceData) > 1">
        <PlatingAirVMod>
          <AirV>
            <xsl:choose>
              ...
              <xsl:when test="../../OTA_AirBookModifyRQ">
                <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:choose>
                  <xsl:when test="@FlightRefNumberRPHList != ''">
                    ...
                    <xsl:value-of 	select="../../OTA_AirBookRQ/AirItinerary/OriginDestinationOptions/OriginDestinationOption/FlightSegment[@RPH=$airseg]/MarketingAirline/@Code"/>
                  </xsl:when>
                  <xsl:otherwise>
      ...
      <xsl:choose>
        ...
        <xsl:when test="@FlightRefNumberRPHList">
          ...
          <SegSelection>
            ...
            <SegRangeAry>
              <xsl:call-template name="GetSegRange">
  ...
  <xsl:template name="GetSegNum">
    ...
    <xsl:if test="$segs != ''">
      ...
      <SegNum>
        <xsl:value-of select="format-number(substring-before($seg,' '),'000')"/>
      </SegNum>
      <xsl:call-template name="GetSegNum">
        <xsl:with-param name="segs">
  ...
  <xsl:template name="GetSegRange">
    ...
    <xsl:if test="$segs != ''">
      ...
      <SegRange>
        <StartSeg>
          <xsl:value-of select="format-number(substring-before($seg,' '),'00')"/>
        </StartSeg>
        <EndSeg>
          <xsl:value-of select="format-number(substring-before($seg,' '),'00')"/>
        </EndSeg>
  ...
  <xsl:template match="NegotiatedFareCode">
    <SegSelection>
      ...
      <SegRangeAry>
        <xsl:call-template name="GetSegRangePriv">
          <xsl:with-param name="segs">
            <xsl:value-of select="../../../@FlightRefNumberRPHList"/>
          </xsl:with-param>
        </xsl:call-template>
  ...
  <xsl:template name="GetSegRangePriv">
    <xsl:param name="segs"/>
    <xsl:if test="$segs != ''">
      <xsl:variable name="seg">
        <xsl:value-of select="concat($segs,' ')"/>
  ...
  <xsl:template match="PriceData" mode="priceagain">
    <xsl:call-template name="CreatePrice">
      <xsl:with-param name="segs">
        <xsl:value-of select="@FlightRefNumberRPHList"/>
      </xsl:with-param>
    </xsl:call-template>
  ...
  <xsl:template name="CreatePrice">
    <xsl:param name="segs"/>
    <xsl:if test="$segs != ''">
      <xsl:variable name="seg">
        <xsl:value-of select="substring-before(concat($segs,' '),' ')"/>
      </xsl:variable>
      <StorePriceMods>
        <xsl:choose>
          <xsl:when test="PassengerTypeQuantity/@Code!=''">
        ...
        <xsl:if test="@FlightRefNumberRPHList !=''">
          <AssocSegs>
            <SegNumAry>
              <SegNum>
                <xsl:value-of select="format-number($seg,'000')"/>
              </SegNum>
            </SegNumAry>
          </AssocSegs>
        </xsl:if>
          ...
          <xsl:otherwise>
            ...
            <SegSelection>
              ...
              <SegRangeAry>
                <SegRange>
                  <StartSeg>
                    <xsl:value-of select="format-number($seg,'00')"/>
                  </StartSeg>
                  <EndSeg>
                    <xsl:value-of select="format-number($seg,'00')"/>
                  </EndSeg>
