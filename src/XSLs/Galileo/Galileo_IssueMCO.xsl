  ...
  -->
					...
					<xsl:if test="Ticketing/TicketDesignators!=''">
						<TicketDesignator>
							<SegNumAry>
								<SegNumInfo>
									<SegNum>0</SegNum>
									<!--SegNum should always be 0 -->
									<TkDesignator>
										<xsl:value-of select="Ticketing/TicketDesignators[1]" />
									</TkDesignator>
								</SegNumInfo>
							</SegNumAry>
						</TicketDesignator>
	...
	<xsl:template name="SSRPerPax">
		...
		<xsl:if test="string-length($TRPH) != 0">
			...
			<xsl:variable name="pax" select="//PNR/PNRBFManagement_53/PNRBFRetrieve/NameRmkInfo" />
			...
			<xsl:for-each select="msxsl:node-set($elems)/elem">
				...
				<Psgr>
					<LNameNum>
						<xsl:choose>
							<xsl:when test="$pax != ''">
								<xsl:value-of select="concat('0',$pax[AbsNameNum=$tRPH]/LNameNum)" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="concat('0',$tRPH)" />
							</xsl:otherwise>
						</xsl:choose>
					</LNameNum>
					<PsgrNum>
						<xsl:choose>
							<xsl:when test="$pax != ''">
								<xsl:value-of select="concat('0',$pax[AbsNameNum=$tRPH]/PsgrNum)" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="concat('0',$tRPH)" />
							</xsl:otherwise>
