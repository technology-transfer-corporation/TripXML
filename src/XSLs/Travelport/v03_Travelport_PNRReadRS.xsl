...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:air="http://www.travelport.com/schema/air_v28_0" xmlns:common_v28_0="http://www.travelport.com/schema/common_v28_0" xmlns:universal="http://www.travelport.com/schema/universal_v28_0" xmlns:SOAP="http://schemas.xmlsoap.org/soap/envelope/" version="1.0">
	...
	<xsl:template match="universal:UniversalRecord">
		<OTA_TravelItineraryRS>
			...
			<xsl:if test="@Version!=''">
				<xsl:attribute name="SequenceNmbr"><xsl:value-of select="@Version"/></xsl:attribute>
			</xsl:if>
				...
				<xsl:when test="@LocatorCode!=''">
					...
					<TravelItinerary>
						<ItineraryRef>
							...
							<xsl:attribute name="ID"><xsl:value-of select="@LocatorCode"/></xsl:attribute>
							<xsl:if test="universal:ProviderReservationInfo/@OwningPCC != ''">
								<xsl:attribute name="ID_Context"><xsl:value-of select="universal:ProviderReservationInfo/@OwningPCC"/></xsl:attribute>
							</xsl:if>
							<CompanyName>
								...
								<xsl:choose>
									...
									<xsl:otherwise><xsl:value-of select="'GAL'"/></xsl:otherwise>
								</xsl:choose>
							</CompanyName>
						</ItineraryRef>
						<CustomerInfos>
							<xsl:apply-templates select="common_v28_0:BookingTraveler"/>
						</CustomerInfos>
						<ItineraryInfo>
							...
							<xsl:apply-templates select="common_v28_0:ActionStatus[@TicketDate!='']" mode="ticketing"/>
							<SpecialRequestDetails>
								...
								<xsl:if test="common_v28_0:GeneralRemark">
									<Remarks>
										<xsl:apply-templates select="common_v28_0:GeneralRemark[@TypeInGds!='Historical']" mode="GenRemark"/>
									</Remarks>
								</xsl:if>
								<xsl:if test="common_v28_0:GeneralRemark[@TypeInGds='Historical']">
									<SpecialRemarks>
										<xsl:apply-templates select="common_v28_0:GeneralRemark[@TypeInGds='Historical']" mode="HistoricalRemark"/>
						...
						<xsl:if test="air:AirReservation/common_v28_0:FormOfPayment or dataElementsMaster/dataElementsIndiv[elementManagementData/segmentName='MCO']">
							<TravelCost>
								<xsl:apply-templates select="air:AirReservation/common_v28_0:FormOfPayment" mode="Payment"/>
						...
						<xsl:if test="dataElementsMaster/dataElementsIndiv[elementManagementData/segmentName='FM'] or common_v28_0:AccountingRemark">
							<TPA_Extensions>
								<xsl:apply-templates select="dataElementsMaster/dataElementsIndiv[elementManagementData/segmentName='FM']" mode="commission"/>
								<xsl:apply-templates select="common_v28_0:AccountingRemark" mode="accounting"/>
							</TPA_Extensions>
						</xsl:if>
					</TravelItinerary>
				</xsl:when>
	...
	<xsl:template name="AirPricingInfo">
		<AirFareInfo>
			...
			<xsl:variable name="bf">
				<xsl:apply-templates select="air:AirReservation/air:AirPricingInfo[1]" mode="totalbase">
					<xsl:with-param name="sum">0</xsl:with-param>
					<xsl:with-param name="pos">1</xsl:with-param>
				</xsl:apply-templates>
			</xsl:variable>
			<xsl:variable name="tf">
				<xsl:apply-templates select="air:AirReservation/air:AirPricingInfo[1]" mode="totalprice">
					<xsl:with-param name="sum">0</xsl:with-param>
					<xsl:with-param name="pos">1</xsl:with-param>
				</xsl:apply-templates>
			</xsl:variable>
			<xsl:variable name="Taxf">
				<xsl:apply-templates select="air:AirReservation/air:AirPricingInfo[1]" mode="totalTax">
					<xsl:with-param name="sum">0</xsl:with-param>
					<xsl:with-param name="pos">1</xsl:with-param>
				</xsl:apply-templates>
			</xsl:variable>
			<xsl:variable name="curt">
				<xsl:value-of select="substring(air:AirReservation/air:AirPricingInfo[1]/@BasePrice,1,3)"/>
			</xsl:variable>
			<xsl:variable name="dect">
				<xsl:value-of select="string-length(substring-after(substring(air:AirReservation/air:AirPricingInfo[1]/@BasePrice,4),'.'))"/>
			</xsl:variable>
			<ItinTotalFare>
				<BaseFare>
			...
			<PTC_FareBreakdowns>
				<xsl:apply-templates select="air:AirReservation/air:AirPricingInfo"/>
			</PTC_FareBreakdowns>
		</AirFareInfo>
	</xsl:template>
		...
		<CustomerInfo>
			...
			<Customer>
				<xsl:if test="dateOfBirth/dateAndTimeDetails[qualifier='706']">
					<xsl:attribute name="BirthDate"><xsl:value-of select="substring(dateOfBirth/dateAndTimeDetails/date,5,4)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(dateOfBirth/dateAndTimeDetails/date,3,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(dateOfBirth/dateAndTimeDetails/date,1,2)"/></xsl:attribute>
				</xsl:if>
				<PersonName>
					...
					<GivenName>
						<xsl:value-of select="common_v28_0:BookingTravelerName/@First"/>
					</GivenName>
					<Surname>
						<xsl:value-of select="common_v28_0:BookingTravelerName/@Last"/>
					</Surname>
				</PersonName>
				<xsl:variable name="paxref">
					<xsl:value-of select="../elementManagementPassenger/reference/number"/>
				</xsl:variable>
				<xsl:apply-templates select="common_v28_0:PhoneNumber" mode="phone"/>
			...
			<CustomerInfo>
				...
				<Customer>
					...
					<PersonName>
						...
						<Surname>
							<xsl:choose>
								<xsl:when test="travellerInformation/traveller/surname!=''"><xsl:value-of select="travellerInformation/traveller/surname"/></xsl:when>
								<xsl:otherwise><xsl:value-of select="../passengerData[1]/travellerInformation/traveller/surname"/></xsl:otherwise>
							</xsl:choose>
						</Surname>
					</PersonName>
				</Customer>
	...
	<xsl:template match="air:AirSegment" mode="Air">
		...
		<Item>
			...
			<xsl:attribute name="ItinSeqNumber"><xsl:value-of select="@TravelOrder"/></xsl:attribute>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<Air>
						...
						<xsl:attribute name="DepartureDateTime"><xsl:value-of select="@DepartureTime"/></xsl:attribute>
						<xsl:attribute name="ArrivalDateTime"><xsl:value-of select="@ArrivalTime"/></xsl:attribute>
						<xsl:attribute name="StopQuantity"><xsl:value-of select="count(air:FlightDetails) - 1"/></xsl:attribute>
						<xsl:attribute name="RPH"><xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="FlightNumber"><xsl:value-of select="@FlightNumber"/></xsl:attribute>
						<xsl:attribute name="ResBookDesigCode"><xsl:value-of select="@ClassOfService"/></xsl:attribute>
						...
						<xsl:if test="@ETicketability != ''">
							<xsl:attribute name="E_TicketEligibility"><xsl:choose><xsl:when test="@ETicketability = 'Yes'">Eligible</xsl:when><xsl:otherwise>NotEligible</xsl:otherwise></xsl:choose></xsl:attribute>
						</xsl:if>
						<DepartureAirport>
							<xsl:attribute name="LocationCode"><xsl:value-of select="@Origin"/></xsl:attribute>
							<xsl:if test="flightDetail/departureInformation/departTerminal != ''">
								<xsl:attribute name="Terminal"><xsl:value-of select="flightDetail/departureInformation/departTerminal"/></xsl:attribute>
							</xsl:if>
						</DepartureAirport>
						<ArrivalAirport>
							<xsl:attribute name="LocationCode"><xsl:value-of select="@Destination"/></xsl:attribute>
						</ArrivalAirport>
						<OperatingAirline>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:attribute name="Code"><xsl:value-of select="@Carrier"/></xsl:attribute>
								</xsl:otherwise>
							</xsl:choose>
						</OperatingAirline>
						<Equipment>
							<xsl:attribute name="AirEquipType"><xsl:value-of select="air:FlightDetails/@Equipment"/></xsl:attribute>
						</Equipment>
						<MarketingAirline>
							<xsl:attribute name="Code"><xsl:value-of select="@Carrier"/></xsl:attribute>
						</MarketingAirline>
						<TPA_Extensions>
							<xsl:attribute name="ConfirmationNumber">
								...
								<xsl:choose>
									<xsl:when test="../common_v28_0:SupplierLocator[@SupplierCode=$airline]">
										<xsl:value-of select="../common_v28_0:SupplierLocator[@SupplierCode=$airline]/@SupplierLocatorCode"/>
									</xsl:when>
	...
	<xsl:template match="itineraryInfo" mode="Hotel">
		<Item>
			...
			<xsl:attribute name="ItinSeqNumber"><xsl:value-of select="elementManagementItinerary[reference/qualifier='ST']/lineNumber"/></xsl:attribute>
				...
				<xsl:choose>
					<xsl:when test="elementManagementItinerary/segmentName='HU'">
						<Reservation>
							<GuestCounts>
								<GuestCount>
									<xsl:attribute name="Count"><xsl:value-of select="relatedProduct/quantity"/></xsl:attribute>
								</GuestCount>
							</GuestCounts>
							<TimeSpan>
								<xsl:attribute name="Start"><xsl:text>20</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,5,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,3,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,1,2)"/></xsl:attribute>
								<xsl:attribute name="End"><xsl:text>20</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,5,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,3,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,1,2)"/></xsl:attribute>
							</TimeSpan>
							<BasicPropertyInfo>
								...
								<xsl:attribute name="HotelName"><xsl:choose><xsl:when test="elementManagementItinerary/segmentName='HU' and contains(itineraryFreetext/longFreetext,',')"><xsl:value-of select="substring-before(substring-after(itineraryFreetext/longFreetext,','),',')"/></xsl:when><xsl:when test="elementManagementItinerary/segmentName='HU' and contains(itineraryFreetext/longFreetext,'-')"><xsl:value-of select="substring-before(itineraryFreetext/longFreetext,'-')"/></xsl:when><xsl:otherwise><xsl:value-of select="substring-before(itineraryFreetext/longFreetext,'/')"/></xsl:otherwise></xsl:choose></xsl:attribute>
							</BasicPropertyInfo>
						</Reservation>
					</xsl:when>
					<xsl:otherwise>
						<Reservation>
							<RoomTypes>
								<RoomType>
									<xsl:attribute name="RoomTypeCode"><xsl:choose><xsl:when test="hotelProduct/hotelRoom/typeCode!=''"><xsl:value-of select="hotelProduct/hotelRoom/typeCode"/></xsl:when><xsl:when test="hotelReservationInfo/roomRateDetails/roomInformation/roomTypeOverride!=''"><xsl:value-of select="hotelReservationInfo/roomRateDetails/roomInformation/roomTypeOverride"/></xsl:when><xsl:otherwise><xsl:value-of select="generalOption/optionDetail[type='RO']/freetext"/></xsl:otherwise></xsl:choose></xsl:attribute>
							...
							<RoomRates>
								<RoomRate>
									<xsl:if test="rateInformations/rateInfo/ratePlan!=''">
										<xsl:attribute name="RatePlanType"><xsl:choose><xsl:when test="rateInformations/rateInfo/ratePlan='DY'">Daily</xsl:when><xsl:when test="rateInformations/rateInfo/ratePlan='PER'">Period</xsl:when><xsl:otherwise>Package</xsl:otherwise></xsl:choose></xsl:attribute>
									</xsl:if>
									<xsl:if test="generalOption[optionDetail/type='BC']">
										<xsl:attribute name="RatePlanID"><xsl:value-of select="generalOption[optionDetail/type='BC']/optionDetail/freetext"/></xsl:attribute>
									</xsl:if>
									<Rates>
										<Rate>
												...
												<xsl:choose>
													<xsl:when test="elementManagementItinerary/segmentName='HU'">
														<xsl:if test="contains(itineraryFreetext/longFreetext,'/AO-')">
															...
															<xsl:attribute name="AmountBeforeTax"><xsl:value-of select="translate(substring($amt,4),'.','')"/></xsl:attribute>
															<xsl:attribute name="CurrencyCode"><xsl:value-of select="substring($amt,1,3)"/></xsl:attribute>
															<xsl:attribute name="DecimalPlaces"><xsl:value-of select="string-length(substring-after($amt,'.'))"/></xsl:attribute>
														</xsl:if>
													</xsl:when>
													<xsl:when test="hotelReservationInfo/roomRateDetails/tariffDetails/tariffInfo/amount != ''">
														...
														<xsl:attribute name="AmountBeforeTax"><xsl:value-of select="translate($amt,'.','')"/></xsl:attribute>
														<xsl:attribute name="CurrencyCode"><xsl:value-of select="hotelReservationInfo/roomRateDetails/tariffDetails/tariffInfo/currency"/></xsl:attribute>
														<xsl:attribute name="DecimalPlaces"><xsl:value-of select="string-length(substring-after($amt,'.'))"/></xsl:attribute>
													</xsl:when>
													<xsl:otherwise>
							...
							<TimeSpan>
								...
								<xsl:if test="generalOption[optionDetail/type='NGT']">
									<xsl:attribute name="Duration"><xsl:value-of select="generalOption[optionDetail/type='NGT']/optionDetail/freetext"/></xsl:attribute>
								</xsl:if>
								<xsl:attribute name="End"><xsl:text>20</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,5,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,3,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,1,2)"/></xsl:attribute>
							</TimeSpan>
							<xsl:choose>
											...
											<GuaranteeAccepted>
												<xsl:choose>
													<xsl:when test="hotelReservationInfo/guaranteeOrDeposit/paymentInfo/paymentDetails/formOfPaymentCode='1'">
														<PaymentCard>
															<xsl:attribute name="CardType"><xsl:value-of select="hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/type"/></xsl:attribute>
															<xsl:attribute name="CardCode"><xsl:choose><xsl:when test="substring(hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/vendorCode,1,2) = 'CA'">MC</xsl:when><xsl:when test="substring(hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/vendorCode,1,2) = 'MA'">MC</xsl:when><xsl:when test="substring(hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/vendorCode,1,2) = 'DC'">DN</xsl:when><xsl:otherwise><xsl:value-of select="substring(hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/vendorCode,1,2)"/></xsl:otherwise></xsl:choose></xsl:attribute>
															<xsl:attribute name="CardNumber"><xsl:value-of select="hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/creditCardNumber"/></xsl:attribute>
															<xsl:attribute name="ExpireDate"><xsl:value-of select="hotelReservationInfo/guaranteeOrDeposit/creditCardInfo/formOfPayment/expiryDate"/></xsl:attribute>
														</PaymentCard>
													</xsl:when>
													<xsl:otherwise>
								...
								<xsl:when test="generalOption[optionDetail/type='G']">
									<Guarantee>
										<GuaranteesAccepted>
											<GuaranteeAccepted>
												<PaymentCard>
													<xsl:attribute name="CardType"><xsl:value-of select="substring(generalOption[optionDetail/type='G']/optionDetail/freetext,1,2)"/></xsl:attribute>
													<xsl:attribute name="CardCode"><xsl:choose><xsl:when test="substring(substring(generalOption[optionDetail/type='G']/optionDetail/freetext,3,2),1,2) = 'CA'">MC</xsl:when><xsl:when test="substring(substring(generalOption[optionDetail/type='G']/optionDetail/freetext,3,2),1,2) = 'MA'">MC</xsl:when><xsl:when test="substring(substring(generalOption[optionDetail/type='G']/optionDetail/freetext,3,2),1,2) = 'DC'">DN</xsl:when><xsl:otherwise><xsl:value-of select="substring(substring(generalOption[optionDetail/type='G']/optionDetail/freetext,3,2),1,2)"/></xsl:otherwise></xsl:choose></xsl:attribute>
													<xsl:attribute name="CardNumber"><xsl:value-of select="substring-before(substring(generalOption[optionDetail/type='G']/optionDetail/freetext,5),'EXP')"/></xsl:attribute>
													<xsl:attribute name="ExpireDate"><xsl:value-of select="substring-after(generalOption[optionDetail/type='G']/optionDetail/freetext,'EXP')"/></xsl:attribute>
												</PaymentCard>
											</GuaranteeAccepted>
										</GuaranteesAccepted>
									</Guarantee>
							...
							<BasicPropertyInfo>
								...
								<xsl:attribute name="HotelName"><xsl:choose><xsl:when test="elementManagementItinerary/segmentName='HU'"><xsl:value-of select="substring-before(itineraryFreetext/longFreetext,'/')"/></xsl:when><xsl:otherwise><xsl:value-of select="hotelProduct/property/name"/></xsl:otherwise></xsl:choose></xsl:attribute>
								<xsl:attribute name="ChainName"><xsl:value-of select="hotelProduct/property/providerName"/></xsl:attribute>
							</BasicPropertyInfo>
						</Reservation>
						<TPA_Extensions>
							<xsl:if test="generalOption[optionDetail/type='BS']">
								<xsl:attribute name="BookingSource"><xsl:value-of select="generalOption[optionDetail/type='BS']/optionDetail/freetext"/></xsl:attribute>
							</xsl:if>
							<xsl:attribute name="ConfirmationNumber"><xsl:choose><xsl:when test="elementManagementItinerary/segmentName='HU'"><xsl:if test="contains(itineraryFreetext/longFreetext,'/CF-')"><xsl:value-of select="substring-before(substring-after(itineraryFreetext/longFreetext,'/CF-'),'/')"/></xsl:if></xsl:when><xsl:otherwise><xsl:value-of select="generalOption/optionDetail[type='CF']/freetext"/></xsl:otherwise></xsl:choose></xsl:attribute>
						</TPA_Extensions>
					</xsl:otherwise>
				</xsl:choose>
			</Hotel>
		...
		<Item>
			...
			<Vehicle>
				<ConfID>
					...
					<xsl:attribute name="ID"><xsl:choose><xsl:when test="generalOption/optionDetail[type='CF']/freetext != ''"><xsl:value-of select="generalOption/optionDetail[type='CF']/freetext"/></xsl:when><xsl:when test="typicalCarData/cancelOrConfirmNbr/reservation[controlType='3' or controlType='2']/controlNumber!= ''"><xsl:value-of select="typicalCarData/cancelOrConfirmNbr/reservation[controlType='3' or controlType='2']/controlNumber"/></xsl:when><xsl:when test="generalOption/optionDetail[type='BS']/freetext!=''"><xsl:value-of select="generalOption/optionDetail[type='BS']/freetext"/></xsl:when><xsl:when test="contains(itineraryFreetext/longFreetext,'/CF-')"><xsl:value-of select="substring-after(itineraryFreetext/longFreetext,'/CF-')"/></xsl:when><xsl:otherwise><xsl:text>UNKNOWN</xsl:text></xsl:otherwise></xsl:choose></xsl:attribute>
				</ConfID>
				<Vendor>
					<xsl:attribute name="Code"><xsl:value-of select="travelProduct/companyDetail/identification"/></xsl:attribute>
				</Vendor>
				<VehRentalCore>
					<xsl:attribute name="PickUpDateTime"><xsl:choose><xsl:when test="elementManagementItinerary/segmentName='CU'"><xsl:text>20</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,5)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,3,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,1,2)"/><xsl:text>T00:00:00</xsl:text></xsl:when><xsl:otherwise><xsl:value-of select="typicalCarData/pickupDropoffTimes/beginDateTime/year"/><xsl:text>-</xsl:text><xsl:value-of select="format-number(typicalCarData/pickupDropoffTimes/beginDateTime/month,'00')"/><xsl:text>-</xsl:text><xsl:value-of select="format-number(typicalCarData/pickupDropoffTimes/beginDateTime/day,'00')"/><xsl:text>T</xsl:text><xsl:choose><xsl:when test="typicalCarData/pickupDropoffTimes/beginDateTime/hour != ''"><xsl:value-of select="format-number(typicalCarData/pickupDropoffTimes/beginDateTime/hour,'00')"/></xsl:when><xsl:otherwise>00</xsl:otherwise></xsl:choose><xsl:text>:</xsl:text><xsl:choose><xsl:when test="typicalCarData/pickupDropoffTimes/beginDateTime/minutes != ''"><xsl:value-of select="format-number(typicalCarData/pickupDropoffTimes/beginDateTime/minutes,'00')"/></xsl:when><xsl:otherwise>00</xsl:otherwise></xsl:choose><xsl:text>:00</xsl:text></xsl:otherwise></xsl:choose></xsl:attribute>
					<xsl:attribute name="ReturnDateTime"><xsl:choose><xsl:when test="elementManagementItinerary/segmentName='CU'"><xsl:text>20</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,5)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,3,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,1,2)"/><xsl:text>T00:00:00</xsl:text></xsl:when><xsl:otherwise><xsl:value-of select="typicalCarData/pickupDropoffTimes/endDateTime/year"/><xsl:text>-</xsl:text><xsl:value-of select="format-number(typicalCarData/pickupDropoffTimes/endDateTime/month,'00')"/><xsl:text>-</xsl:text><xsl:value-of select="format-number(typicalCarData/pickupDropoffTimes/endDateTime/day,'00')"/><xsl:text>T</xsl:text><xsl:choose><xsl:when test="typicalCarData/pickupDropoffTimes/endDateTime/hour!=''"><xsl:value-of select="format-number(typicalCarData/pickupDropoffTimes/endDateTime/hour,'00')"/><xsl:text>:</xsl:text><xsl:value-of select="format-number(typicalCarData/pickupDropoffTimes/endDateTime/minutes,'00')"/><xsl:text>:00</xsl:text></xsl:when><xsl:otherwise><xsl:text>00:00:00</xsl:text></xsl:otherwise></xsl:choose></xsl:otherwise></xsl:choose></xsl:attribute>
				...
				<Veh>
					<xsl:attribute name="AirConditionInd"><xsl:choose><xsl:when test="substring(travelProduct/productDetails/identification,4,1) = 'R'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose></xsl:attribute>
					<xsl:attribute name="TransmissionType"><xsl:choose><xsl:when test="substring(travelProduct/productDetails/identification,3,1) = 'A'">Automatic</xsl:when><xsl:otherwise>Manual</xsl:otherwise></xsl:choose></xsl:attribute>
					<VehType>
						<xsl:attribute name="VehicleCategory"><xsl:choose><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'C'">2/4 Door Car</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'B'">2-Door Car</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'D'">4-Door Car</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'W'">Wagon</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'V'">Van</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'L'">Limousine</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'S'">Sport</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'T'">Convertible</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'F'">4-Wheel Drive</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'P'">Pickup</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,2,1) = 'J'">All-Terrain</xsl:when><xsl:otherwise>Unavailable</xsl:otherwise></xsl:choose></xsl:attribute>
						<xsl:value-of select="travelProduct/productDetails/identification"/>
					</VehType>
					<VehClass>
						<xsl:attribute name="Size"><xsl:choose><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'M'">Mini</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'E'">Economy</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'C'">Compact</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'I'">Intermediate</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'S'">Standard</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'F'">Full-Size</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'P'">Premium</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'L'">Luxury</xsl:when><xsl:when test="substring(travelProduct/productDetails/identification,1,1) = 'X'">Special</xsl:when><xsl:otherwise>Unavailable</xsl:otherwise></xsl:choose></xsl:attribute>
					</VehClass>
				</Veh>
				<xsl:if test="elementManagementItinerary/segmentName!='CU'">
					<RentalRate>
						<xsl:choose>
							...
							<xsl:otherwise>
								...
								<xsl:if test="$rateText!=''">
									<RateDistance>
										<xsl:attribute name="Unlimited"><xsl:choose><xsl:when test="contains($rateText, 'UNL')">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose></xsl:attribute>
										<xsl:attribute name="DistUnitName"><xsl:choose><xsl:when test="contains($rateText, ' KM')">Km</xsl:when><xsl:otherwise>Mile</xsl:otherwise></xsl:choose></xsl:attribute>
									</RateDistance>
									<VehicleCharges>
										<VehicleCharge>
											...
											<xsl:attribute name="Amount"><xsl:choose><xsl:when test="starts-with($rateText,$curr)"><xsl:variable name="rateText1"><xsl:choose><xsl:when test="contains($rateText,'-')"><xsl:value-of select="substring-before($rateText,'-')"/></xsl:when><xsl:otherwise><xsl:value-of select="$rateText"/></xsl:otherwise></xsl:choose></xsl:variable><xsl:value-of select="translate(substring-before(substring($rateText1,4),'.'),'ABCDEFGHIJKLMNOPQRSTUVWXYZ*','')"/><xsl:choose><xsl:when test="number(substring(substring-after(substring($rateText1,4),'.'),2,1))"><xsl:value-of select="substring(substring-after(substring($rateText1,4),'.'),1,2)"/></xsl:when><xsl:otherwise><xsl:value-of select="concat(substring(substring-after(substring($rateText1,4),'.'),1,1),'0')"/></xsl:otherwise></xsl:choose></xsl:when><xsl:otherwise><xsl:choose><xsl:when test="contains($rateText,'-')"><xsl:variable name="rateText1"><xsl:choose><xsl:when test="contains(substring-before(substring($rateText,8),'-'),'.')"><xsl:value-of select="translate(substring-before(substring($rateText,8),'-'),'.','')"/></xsl:when><xsl:otherwise><xsl:value-of select="concat(substring-before(substring($rateText,8),'-'),'00')"/></xsl:otherwise></xsl:choose></xsl:variable><xsl:value-of select="translate($rateText1,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','')"/></xsl:when><xsl:otherwise><xsl:value-of select="substring-before(substring($rateText,8),'.')"/><xsl:choose><xsl:when test="number(substring(substring-after(substring($rateText,8),'.'),2,1))"><xsl:value-of select="substring(substring-after(substring($rateText,8),'.'),1,2)"/></xsl:when><xsl:otherwise><xsl:value-of select="concat(substring(substring-after(substring($rateText,8),'.'),1,1),'0')"/></xsl:otherwise></xsl:choose></xsl:otherwise></xsl:choose></xsl:otherwise></xsl:choose></xsl:attribute>
											<xsl:attribute name="CurrencyCode"><xsl:choose><xsl:when test="starts-with($rateText,$curr)"><xsl:value-of select="substring($rateText,1,3)"/></xsl:when><xsl:otherwise><xsl:value-of select="substring($rateText,5,3)"/></xsl:otherwise></xsl:choose></xsl:attribute>
											<xsl:attribute name="DecimalPlaces">2</xsl:attribute>
											<xsl:attribute name="TaxInclusive">false</xsl:attribute>
											<xsl:attribute name="GuaranteedInd"><xsl:choose><xsl:when test="typicalCarData/rateInfo/chargeDetails[type='RG']">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose></xsl:attribute>
											<xsl:attribute name="Description"><xsl:choose><xsl:when test="contains($rateText, 'HY')">Hourly Rate</xsl:when><xsl:when test="contains($rateText, 'DY')">Daily Rate</xsl:when><xsl:when test="contains($rateText, 'WY')">Weekly Rate</xsl:when><xsl:when test="contains($rateText, 'MY')">Monthly Rate</xsl:when><xsl:when test="contains($rateText, 'WD')">Weekend Rate</xsl:when><xsl:when test="contains($rateText, 'PK')">Package Rate</xsl:when><xsl:otherwise>Rental Period Rate</xsl:otherwise></xsl:choose></xsl:attribute>
										</VehicleCharge>
									</VehicleCharges>
								</xsl:if>
							</xsl:otherwise>
						...
						<xsl:if test="typicalCarData/rateCodeGroup/rateCodeInfo/fareCategories/fareType != ''">
							<RateQualifier>
								<xsl:if test="typicalCarData/rateInfo/rateInformation/category!=''">
									<xsl:attribute name="RateCategory"><xsl:choose><xsl:when test="typicalCarData/rateInfo/rateInformation/category='2'">Inclusive</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='6'">Convention</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='7'">Corporate</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='9'">Government</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='11'">Package</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='19'">Association</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='20'">Business</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='21'">Consortium</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='22'">Credential</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='23'">Industry</xsl:when><xsl:when test="typicalCarData/rateInfo/rateInformation/category='24'">Standard</xsl:when><xsl:otherwise>General</xsl:otherwise></xsl:choose></xsl:attribute>
								</xsl:if>
								<xsl:attribute name="RateQualifier"><xsl:value-of select="typicalCarData/rateCodeGroup/rateCodeInfo/fareCategories/fareType"/></xsl:attribute>
								<xsl:attribute name="RatePeriod"><xsl:variable name="rateinfo" select="typicalCarData/rateInfo[tariffInfo/amountType='RG' or tariffInfo/amountType='RQ']"/><xsl:choose><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator != ''"><xsl:choose><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'HY'">Hourly</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'DY'">Daily</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'WY'">Weekly</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'MY'">Monthly</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'WD'">WeekendDay</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'PK'">Package</xsl:when><xsl:otherwise>Other</xsl:otherwise></xsl:choose></xsl:when><xsl:when test="contains(generalOption/optionDetail[type='RG']/freetext, 'DY')">Daily</xsl:when><xsl:when test="contains(generalOption/optionDetail[type='RG']/freetext, 'WY')">Weekly</xsl:when><xsl:when test="contains(typicalCarData/rateInfo/chargeDetails[type='RG']/comment, 'HY')">Hourly</xsl:when><xsl:when test="contains(typicalCarData/rateInfo/chargeDetails[type='RG']/comment, 'DY')">Daily</xsl:when><xsl:when test="contains(typicalCarData/rateInfo/chargeDetails[type='RG']/comment, 'WY')">Weekly</xsl:when><xsl:when test="contains(typicalCarData/rateInfo/chargeDetails[type='RG']/comment, 'MY')">Monthly</xsl:when><xsl:when test="contains(typicalCarData/rateInfo/chargeDetails[type='RG']/comment, 'WD')">WeekendDay</xsl:when><xsl:when test="contains(typicalCarData/rateInfo/chargeDetails[type='RG']/comment, 'PK')">Package</xsl:when><xsl:otherwise>Other</xsl:otherwise></xsl:choose></xsl:attribute>
							</RateQualifier>
						</xsl:if>
					</RentalRate>
				</xsl:if>
				...
				<xsl:if test="$total != '.'">
					<TotalCharge>
						...
						<xsl:attribute name="CurrencyCode"><xsl:choose><xsl:when test="typicalCarData/rateInfo/tariffInfo[amountType = 'RB']/currency != ''"><xsl:value-of select="typicalCarData/rateInfo/tariffInfo[amountType = 'RB']/currency"/></xsl:when><xsl:when test="typicalCarData/rateInfo/tariffInfo[amountType = '904']/currency != ''"><xsl:value-of select="typicalCarData/rateInfo/tariffInfo[amountType = '904']/currency"/></xsl:when><xsl:otherwise><xsl:value-of select="substring(generalOption/optionDetail[type='RG']/freetext,5,3)"/></xsl:otherwise></xsl:choose></xsl:attribute>
					</TotalCharge>
				</xsl:if>
				<TPA_Extensions>
					<CarOptions>
	...
	<xsl:template match="typicalCarData">
		...
		<RateDistance>
			<xsl:attribute name="Unlimited"><xsl:choose><xsl:when test="$rateinfo/chargeDetails/description = 'UNL'">true</xsl:when><xsl:when test="contains(generalOption/optionDetail[type='RG']/freetext, 'UNL')">true</xsl:when><xsl:when test="contains($rateinfo/chargeDetails[type='RG' or type='RQ']/comment, 'UNL')">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose></xsl:attribute>
			<xsl:attribute name="DistUnitName"><xsl:choose><xsl:when test="$rateinfo[chargeDetails/type='31']">Mile</xsl:when><xsl:when test="$rateinfo[chargeDetails/type='32']">Km</xsl:when><xsl:otherwise>Mile</xsl:otherwise></xsl:choose></xsl:attribute>
		</RateDistance>
		<VehicleCharges>
			<VehicleCharge>
				<xsl:attribute name="Amount"><xsl:value-of select="translate($rateinfo/tariffInfo/amount,'.','')"/></xsl:attribute>
				<xsl:attribute name="CurrencyCode"><xsl:value-of select="$rateinfo/tariffInfo/currency"/></xsl:attribute>
				<xsl:attribute name="DecimalPlaces"><xsl:value-of select="string-length(substring-after($rateinfo/tariffInfo/amount,'.'))"/></xsl:attribute>
				<xsl:attribute name="TaxInclusive">false</xsl:attribute>
				<xsl:attribute name="Description"><xsl:choose><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'HY'">Hourly Rate</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'DY'">Daily Rate</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'WY'">Weekly Rate</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'MY'">Monthly Rate</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'WD'">Weekend Rate</xsl:when><xsl:when test="$rateinfo/tariffInfo/ratePlanIndicator = 'PK'">Package Rate</xsl:when><xsl:otherwise>Rental Period Rate</xsl:otherwise></xsl:choose></xsl:attribute>
				<xsl:attribute name="GuaranteedInd"><xsl:choose><xsl:when test="$rateinfo/tariffInfo[amountType = 'RG']/amount != ''">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose></xsl:attribute>
			</VehicleCharge>
			<xsl:if test="$rateinfo/chargeDetails[type='8']">
				<VehicleCharge>
					<xsl:attribute name="Amount"><xsl:value-of select="translate($rateinfo/chargeDetails[type='8']/amount,'.','')"/></xsl:attribute>
					<xsl:attribute name="DecimalPlaces"><xsl:value-of select="string-length(substring-after($rateinfo/chargeDetails[type='8']/amount,'.'))"/></xsl:attribute>
					<xsl:attribute name="TaxInclusive">false</xsl:attribute>
					<xsl:attribute name="Description">Extra Day Rate</xsl:attribute>
				</VehicleCharge>
			</xsl:if>
			<xsl:if test="$rateinfo/chargeDetails[type='9']">
				<VehicleCharge>
					<xsl:attribute name="Amount"><xsl:value-of select="translate($rateinfo/chargeDetails[type='9']/amount,'.','')"/></xsl:attribute>
					<xsl:attribute name="DecimalPlaces"><xsl:value-of select="string-length(substring-after($rateinfo/chargeDetails[type='9']/amount,'.'))"/></xsl:attribute>
					<xsl:attribute name="TaxInclusive">false</xsl:attribute>
					<xsl:attribute name="Description">Extra Hour Rate</xsl:attribute>
				</VehicleCharge>
			</xsl:if>
			<xsl:if test="$rateinfo[chargeDetails/type='31'] or $rateinfo[chargeDetails/type='32']">
				<VehicleCharge>
					<xsl:attribute name="Amount"><xsl:value-of select="translate($rateinfo[chargeDetails/type='31' or chargeDetails/type='32']/chargeDetails/amount,'.','')"/></xsl:attribute>
					<xsl:attribute name="DecimalPlaces"><xsl:value-of select="string-length(substring-after($rateinfo[chargeDetails/type='31' or chargeDetails/type='32']/chargeDetails/amount,'.'))"/></xsl:attribute>
					<xsl:attribute name="TaxInclusive">false</xsl:attribute>
	...
	<!-- Phone Fields	   	                                    -->
	<!-- ************************************************************** -->
	<xsl:template match="common_v28_0:PhoneNumber" mode="phone">
		<Telephone>
			<xsl:attribute name="PhoneUseType">
				<xsl:choose>
					<xsl:when test="@Type='4'">H</xsl:when>
					<xsl:when test="@Type='P01'">F</xsl:when>
					<xsl:when test="@Type='6'">A</xsl:when>
					<xsl:when test="@Type='3'">B</xsl:when>
					<xsl:when test="@Type='7'">M</xsl:when>
					<xsl:otherwise>O</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
			<xsl:attribute name="PhoneNumber"><xsl:value-of select="@Number"/></xsl:attribute>
		</Telephone>
	</xsl:template>
	<!-- ************************************************************** -->
	<!-- Ticketing Element	   	                                    -->
	<!-- ************************************************************** -->
	<xsl:template match="common_v28_0:ActionStatus" mode="ticketing">
		<Ticketing>
	...
	<!-- Form of Payment	   		                                    -->
	<!-- ************************************************************** -->
	<xsl:template match="common_v28_0:FormOfPayment" mode="Payment">
			<xsl:choose>
				<xsl:when test="common_v28_0:CreditCard">
					<FormOfPayment>
						<xsl:attribute name="RPH">1</xsl:attribute>
						<PaymentCard>
							<xsl:attribute name="CardCode">
								<xsl:choose>
									<xsl:when test="common_v28_0:CreditCard/@Type = 'CA'">MC</xsl:when>
									<xsl:when test="common_v28_0:CreditCard/@Type = 'DC'">DN</xsl:when>
									<xsl:otherwise><xsl:value-of select="common_v28_0:CreditCard/@Type"/></xsl:otherwise>
								</xsl:choose>
							</xsl:attribute>
							<xsl:attribute name="CardNumber"><xsl:value-of select="common_v28_0:CreditCard/@Number"/></xsl:attribute>
							<xsl:attribute name="ExpireDate"><xsl:value-of select="concat(substring(common_v28_0:CreditCard/@ExpDate,6),substring(common_v28_0:CreditCard/@ExpDate,3,2))"/></xsl:attribute>
						</PaymentCard>
					</FormOfPayment>
				</xsl:when>
			</xsl:choose>
	...
	<xsl:template match="dataElementsIndiv" mode="MCO">
		<FormOfPayment>
			<xsl:attribute name="RPH"><xsl:value-of select="elementManagementData/lineNumber"/></xsl:attribute>
			<MiscChargeOrder>
				<xsl:attribute name="TravelerRefNumber"><xsl:variable name="pax"><xsl:value-of select="referenceForDataElement/reference/number"/></xsl:variable><xsl:value-of select="../../travellerInfo[elementManagementPassenger/reference/number = $pax]/elementManagementPassenger/lineNumber"/></xsl:attribute>
			...
			<xsl:if test="mcoRecord/groupOfFareElements/fareElementData/freeTextDetails/informationType='FM' or (mcoRecord/groupOfFareElements/fareElementData/freeTextDetails/informationType='FA' or mcoRecord/groupOfFareElements/fareElementData/freeTextDetails/informationType='FB' or mcoRecord/groupOfFareElements/fareElementData/freeTextDetails/informationType='FI') or mcoRecord/groupOfFareElements[fareElementData/freeTextDetails/informationType='FP' and starts-with(fareElementData/freeText,'CC') and string-length(fareElementData/freeText) &gt; 10 and substring(fareElementData/freeText,3,2)!='ZZ']">
				<TPA_Extensions>
								...
								<xsl:when test="contains(fareElementData/freeText,'*M*') or contains(fareElementData/freeText,'*C*') or contains(fareElementData/freeText,'*G*') or contains(fareElementData/freeText,'*F*')">
									...
									<xsl:choose>
										<xsl:when test="contains($comm,'A')">
											<xsl:attribute name="Amount"><xsl:value-of select="substring-before($comm,'A')"/></xsl:attribute>
										</xsl:when>
										<xsl:otherwise>
											<xsl:attribute name="Percent"><xsl:value-of select="$comm"/></xsl:attribute>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:when>
								<xsl:when test="contains(fareElementData/freeText,'*AM*')">
									<xsl:attribute name="Amount"><xsl:value-of select="substring-after(fareElementData/freeText,'*AM*')"/></xsl:attribute>
								</xsl:when>
								<xsl:when test="contains(fareElementData/freeText,'A')">
									<xsl:attribute name="Amount"><xsl:value-of select="substring-before(fareElementData/freeText,'A')"/></xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
									<xsl:attribute name="Percent"><xsl:value-of select="fareElementData/freeText"/></xsl:attribute>
								</xsl:otherwise>
							</xsl:choose>
						</AgencyCommission>
					</xsl:for-each>
					...
					<xsl:for-each select="mcoRecord/groupOfFareElements[fareElementData/freeTextDetails/informationType='FP' and starts-with(fareElementData/freeText,'CC') and string-length(fareElementData/freeText) &gt; 10 and substring(fareElementData/freeText,3,2)!='ZZ']">
						<PaymentCard>
							<xsl:attribute name="CardCode"><xsl:choose><xsl:when test="substring(substring-after(fareElementData/freeText,'CC'),1,2) = 'CA'">MC</xsl:when><xsl:when test="substring(substring-after(fareElementData/freeText,'CC'),1,2) = 'MA'">MC</xsl:when><xsl:when test="substring(substring-after(fareElementData/freeText,'CC'),1,2) = 'DC'">DN</xsl:when><xsl:when test="substring(fareElementData/freeText,4,1) = '/'"><xsl:variable name="cc"><xsl:value-of select="substring(fareElementData/freeText,2,2)"/></xsl:variable><xsl:choose><xsl:when test="$cc='CA'">MC</xsl:when><xsl:when test="$cc='MA'">MC</xsl:when><xsl:when test="$cc='DC'">DN</xsl:when><xsl:otherwise><xsl:value-of select="$cc"/></xsl:otherwise></xsl:choose></xsl:when><xsl:when test="substring(fareElementData/freeText,4,1) = '-'"><xsl:variable name="cc"><xsl:value-of select="substring(fareElementData/freeText,2,2)"/></xsl:variable><xsl:choose><xsl:when test="$cc='CA'">MC</xsl:when><xsl:when test="$cc='MA'">MC</xsl:when><xsl:when test="$cc='DC'">DN</xsl:when><xsl:otherwise><xsl:value-of select="$cc"/></xsl:otherwise></xsl:choose></xsl:when><xsl:otherwise><xsl:value-of select="substring(substring-after(fareElementData/freeText,'CC'),1,2)"/></xsl:otherwise></xsl:choose></xsl:attribute>
							<xsl:attribute name="CardNumber"><xsl:variable name="cardnum"><xsl:choose><xsl:when test="substring(fareElementData/freeText,4,1) = '-'"><xsl:value-of select="substring-before(substring-after(fareElementData/freeText,'/'),'T')"/></xsl:when><xsl:otherwise><xsl:value-of select="translate(substring(substring-after(fareElementData/freeText,'CC'),3,16),'/','')"/></xsl:otherwise></xsl:choose></xsl:variable><xsl:value-of select="$cardnum"/></xsl:attribute>
							<xsl:attribute name="ExpireDate"><xsl:choose><xsl:when test="substring(fareElementData/freeText,4,1) = '-'"><xsl:value-of select="substring(substring-after(fareElementData/freeText,'T/'),1,2)"/><xsl:value-of select="substring(substring-after(fareElementData/freeText,'T/'),3,2)"/></xsl:when><xsl:otherwise><xsl:value-of select="substring(substring-after(substring(fareElementData/freeText,5),'/'),1,2)"/><xsl:value-of select="substring(substring-after(substring(fareElementData/freeText,5),'/'),3,2)"/></xsl:otherwise></xsl:choose></xsl:attribute>
							<xsl:if test="contains(substring-after(substring(fareElementData/freeText,5),'/'),'/N')">
								<xsl:attribute name="ConfirmationNumber"><xsl:variable name="cn"><xsl:value-of select="substring-after(substring-after(substring(fareElementData/freeText,5),'/'),'/N')"/></xsl:variable><xsl:choose><xsl:when test="contains($cn,'*')"><xsl:value-of select="concat('N',substring-before($cn,'*'))"/></xsl:when><xsl:otherwise><xsl:value-of select="concat('N',$cn)"/></xsl:otherwise></xsl:choose></xsl:attribute>
							</xsl:if>
						</PaymentCard>
					</xsl:for-each>
				</TPA_Extensions>
	...
	<xsl:template match="dataElementsIndiv" mode="Address">
		<Address>
			<xsl:attribute name="UseType"><xsl:choose><xsl:when test="elementManagementData/segmentName = 'AB/'">Billing</xsl:when><xsl:when test="elementManagementData/segmentName = 'AB'">Billing</xsl:when><xsl:otherwise>Mailing</xsl:otherwise></xsl:choose></xsl:attribute>
			<xsl:choose>
			...
			<StateProv>
				<xsl:choose>
					<xsl:when test="string-length(address[option='ST']/optionText) = 2">
						<xsl:attribute name="StateCode"><xsl:value-of select="address[option='ST']/optionText"/></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
			...
			<CountryName>
				<xsl:choose>
					<xsl:when test="string-length(address[option='CO']/optionText) = 2">
						<xsl:attribute name="Code"><xsl:value-of select="address[option='CO']/optionText"/></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template match="dataElementsIndiv" mode="Fqtv">
		...
		<xsl:if test="referenceForDataElement/reference[qualifier='PT']/number = $paxref">
			<CustLoyalty>
				<xsl:attribute name="ProgramID"><xsl:value-of select="frequentTravellerInfo/frequentTraveler/company"/></xsl:attribute>
				<xsl:attribute name="MembershipID"><xsl:value-of select="frequentTravellerInfo/frequentTraveler/membershipNumber"/></xsl:attribute>
			</CustLoyalty>
		</xsl:if>
	</xsl:template>
	<!-- ************************************************************** -->
	<!-- General Remarks	   	                                    -->
	<!-- ************************************************************** -->
	<xsl:template match="common_v28_0:GeneralRemark" mode="GenRemark">
		<Remark>
			<xsl:attribute name="RPH"><xsl:value-of select="position()"/></xsl:attribute>
			...
			<xsl:value-of select="common_v28_0:RemarkData"/>
		</Remark>
	</xsl:template>
	<!-- ************************************************************** -->
	<!--Itinerary Remarks	   	                                    -->
	...
	<xsl:template match="dataElementsIndiv" mode="ItinRemark">
		<SpecialRemark>
			...
			<FlightRefNumber>
				<xsl:attribute name="RPH"><xsl:value-of select="elementManagementData/reference[qualifier='OT']/number"/></xsl:attribute>
			</FlightRefNumber>
			<Text>
		...
		<SpecialRemark>
			...
			<xsl:choose>
				<xsl:when test="referenceForDataElement/reference/qualifier='ST'">
					<xsl:attribute name="RemarkType"><xsl:value-of select="miscellaneousRemarks/remarks/category"/></xsl:attribute>
					<FlightRefNumber>
						<xsl:attribute name="RPH"><xsl:variable name="rph"><xsl:value-of select="referenceForDataElement/reference[qualifier='ST']/number"/></xsl:variable><xsl:value-of select="../../originDestinationDetails/itineraryInfo/elementManagementItinerary[reference/number = $rph]/lineNumber"/></xsl:attribute>
					</FlightRefNumber>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="RemarkType"><xsl:value-of select="miscellaneousRemarks/remarks/category"/></xsl:attribute>
	...
	<!-- Historical Remarks	   	                              -->
	<!-- ************************************************************** -->
	<xsl:template match="common_v28_0:GeneralRemark" mode="HistoricalRemark">
		<SpecialRemark>
			<xsl:attribute name="RPH"><xsl:value-of select="position()"/></xsl:attribute>
			...
			<xsl:choose>
				<xsl:when test="referenceForDataElement/reference/qualifier='ST' or referenceForDataElement/reference/qualifier='PT'">
					...
					<xsl:if test="referenceForDataElement/reference/qualifier='PT'">
						<TravelerRefNumber>
							<xsl:attribute name="RPH"><xsl:for-each select="referenceForDataElement/reference[qualifier='PT']/number"><xsl:variable name="rph"><xsl:value-of select="."/></xsl:variable><xsl:if test="position() > 1"><xsl:text> </xsl:text></xsl:if><xsl:value-of select="../../../../../travellerInfo/elementManagementPassenger[reference/number = $rph]/lineNumber"/></xsl:for-each></xsl:attribute>
						</TravelerRefNumber>
					</xsl:if>
					<xsl:if test="referenceForDataElement/reference/qualifier='ST'">
						<FlightRefNumber>
							<xsl:attribute name="RPH"><xsl:for-each select="referenceForDataElement/reference[qualifier='ST']/number"><xsl:variable name="rph"><xsl:value-of select="."/></xsl:variable><xsl:if test="position() > 1"><xsl:text> </xsl:text></xsl:if><xsl:value-of select="../../../../../originDestinationDetails/itineraryInfo/elementManagementItinerary[reference/number = $rph]/lineNumber"/></xsl:for-each></xsl:attribute>
						</FlightRefNumber>
					</xsl:if>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template match="dataElementsIndiv" mode="OSI">
		<OtherServiceInformation>
			<xsl:if test="otherDataFreetext/freetextDetail/type='28'">
				<Airline>
					<xsl:attribute name="Code"><xsl:value-of select="otherDataFreetext/freetextDetail/companyId"/></xsl:attribute>
				</Airline>
				<Text>
					<xsl:value-of select="otherDataFreetext/longFreetext"/>
				</Text>
	...
	<xsl:template match="reference" mode="OSIPassAssoc">
		...
		<xsl:attribute name="RPH"><xsl:value-of select="//travellerInfo/elementManagementPassenger[reference/number=$Tattoo]/lineNumber"/></xsl:attribute>
	</xsl:template>
	<!-- ************************************************************** -->
	<!-- SSR Elements 	                               		    -->
	<!-- ************************************************************** -->
	<xsl:template match="dataElementsIndiv" mode="SSR">
		<SpecialServiceRequest>
			<xsl:attribute name="SSRCode"><xsl:value-of select="serviceRequest/ssr/type"/></xsl:attribute>
			<xsl:if test="referenceForDataElement/reference[qualifier = 'PT']/number != ''">
				...
				<xsl:attribute name="TravelerRefNumberRPHList"><xsl:value-of select="../../travellerInfo/elementManagementPassenger[reference/number = $ref]/lineNumber"/></xsl:attribute>
			</xsl:if>
			<xsl:if test="referenceForDataElement/reference[qualifier = 'ST']/number != ''">
				...
				<xsl:attribute name="FlightRefNumberRPHList"><xsl:value-of select="../../originDestinationDetails/itineraryInfo/elementManagementItinerary[reference/number = $ref]/lineNumber"/></xsl:attribute>
			</xsl:if>
			<Airline>
				<xsl:attribute name="Code"><xsl:value-of select="serviceRequest/ssr/companyId"/></xsl:attribute>
			</Airline>
			<Text>
		...
		<xsl:choose>
			<xsl:when test="referenceForDataElement/reference[qualifier='PT']">
				<xsl:for-each select="serviceRequest/ssrb">
					...
					<SeatRequest>
						<xsl:if test="data != ''">
							<xsl:attribute name="SeatNumber"><xsl:value-of select="data"/></xsl:attribute>
						</xsl:if>
						<xsl:if test="seatType[position()=2] != ''">
							<xsl:attribute name="SeatPreference"><xsl:value-of select="seatType[position()=2]"/></xsl:attribute>
						</xsl:if>
						...
						<xsl:attribute name="TravelerRefNumberRPHList"><xsl:variable name="ref"><xsl:value-of select="../../referenceForDataElement/reference[qualifier = 'PT'][position()=$pos]/number"/></xsl:variable><xsl:value-of select="../../../../travellerInfo/elementManagementPassenger[reference/number = $ref]/lineNumber"/><!--xsl:apply-templates select="../../referenceForDataElement/reference[qualifier='PT']" mode="SeatPassAssoc"/--></xsl:attribute>
						<xsl:attribute name="FlightRefNumberRPHList"><xsl:apply-templates select="../../referenceForDataElement/reference[qualifier='ST']" mode="SeatSegAssoc"/></xsl:attribute>
						<DepartureAirport>
			...
			<xsl:otherwise>
				<xsl:for-each select="serviceRequest/ssrb[data != '']">
					<SeatRequest>
						...
						<xsl:attribute name="SmokingAllowed">false</xsl:attribute>
						<xsl:attribute name="Status"><xsl:value-of select="../ssr/status"/></xsl:attribute>
						<xsl:attribute name="TravelerRefNumberRPHList"><xsl:value-of select="position()"/></xsl:attribute>
						<xsl:attribute name="FlightRefNumberRPHList"><xsl:apply-templates select="../../referenceForDataElement/reference[qualifier='ST']" mode="SeatSegAssoc"/></xsl:attribute>
						<DepartureAirport>
							<xsl:attribute name="LocationCode"><xsl:value-of select="../ssr/boardpoint"/></xsl:attribute>
						</DepartureAirport>
	...
	<xsl:template match="dataElementsIndiv" mode="IssuedTicket">
		<IssuedTicket>
			<xsl:if test="referenceForDataElement/reference/qualifier='PT'">
				<xsl:attribute name="TravelerRefNumberRPHList"><xsl:for-each select="referenceForDataElement/reference[qualifier='PT']/number"><xsl:variable name="rph"><xsl:value-of select="."/></xsl:variable><xsl:if test="position() > 1"><xsl:text> </xsl:text></xsl:if><xsl:value-of select="../../../../../travellerInfo/elementManagementPassenger[reference/number = $rph]/lineNumber"/></xsl:for-each></xsl:attribute>
			</xsl:if>
			<xsl:if test="referenceForDataElement/reference/qualifier='ST'">
				<xsl:attribute name="FlightRefNumberRPHList"><xsl:for-each select="referenceForDataElement/reference[qualifier='ST']/number"><xsl:variable name="rph"><xsl:value-of select="."/></xsl:variable><xsl:if test="position() > 1"><xsl:text> </xsl:text></xsl:if><xsl:value-of select="../../../../../originDestinationDetails/itineraryInfo/elementManagementItinerary[reference/number = $rph]/lineNumber"/></xsl:for-each></xsl:attribute>
			</xsl:if>
			<xsl:value-of select="otherDataFreetext/longFreetext"/>
		</IssuedTicket>
	</xsl:template>
	...
	<xsl:template match="reference" mode="SSRPassAssoc">
		...
		<xsl:attribute name="TravelerRefNumberRPHList"><xsl:value-of select="//travellerInfo/elementManagementPassenger[reference/number=$Tattoo]/lineNumber"/></xsl:attribute>
	</xsl:template>
		...
		<AgencyCommission>
			...
			<xsl:choose>
				<xsl:when test="contains(otherDataFreetext/longFreetext,'*M*') or contains(otherDataFreetext/longFreetext,'*C*') or contains(otherDataFreetext/longFreetext,'*G*') or contains(otherDataFreetext/longFreetext,'*F*')">
					...
					<xsl:choose>
						<xsl:when test="contains($comm,'A')">
							<xsl:attribute name="Amount"><xsl:choose><xsl:when test="contains(substring-after($comm,'.'),'.')"><xsl:choose><xsl:when test="contains(substring-before(substring-after($comm,'.'),'.'),'A')"><xsl:value-of select="substring-before($comm,'A')"/></xsl:when><xsl:otherwise><xsl:value-of select="concat(substring-before($comm,'.'),'.')"/><xsl:value-of select="substring-before(substring-after($comm,'.'),'.')"/></xsl:otherwise></xsl:choose></xsl:when><xsl:otherwise><xsl:value-of select="substring-before($comm,'A')"/></xsl:otherwise></xsl:choose></xsl:attribute>
						</xsl:when>
						<xsl:when test="contains($comm,'N')">
							<xsl:attribute name="Percent"><xsl:value-of select="translate(substring-before($comm,'N'),'ABCDEFGHIJKLMNOPQRSTUVWXYZ* ','')"/></xsl:attribute>
						</xsl:when>
						<xsl:otherwise>
							<xsl:attribute name="Percent"><xsl:value-of select="translate($comm,'ABCDEFGHIJKLMNOPQRSTUVWXYZ* ','')"/></xsl:attribute>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:when test="contains(otherDataFreetext/longFreetext,'*AM*')">
					<xsl:attribute name="Amount"><xsl:value-of select="translate(substring-after(otherDataFreetext/longFreetext,'*AM*'),'A','')"/></xsl:attribute>
				</xsl:when>
				<xsl:when test="contains(otherDataFreetext/longFreetext,'*PR*')">
					<xsl:attribute name="Percent"><xsl:choose><xsl:when test="contains(substring-after(otherDataFreetext/longFreetext,'*PR*'),'/')"><xsl:value-of select="substring-before(substring-after(otherDataFreetext/longFreetext,'*PR*'),'/')"/></xsl:when><xsl:otherwise><xsl:value-of select="substring-after(otherDataFreetext/longFreetext,'*PR*')"/></xsl:otherwise></xsl:choose></xsl:attribute>
				</xsl:when>
				<xsl:when test="contains(otherDataFreetext/longFreetext,'*P*')">
					<xsl:attribute name="Percent"><xsl:choose><xsl:when test="contains(substring-after(otherDataFreetext/longFreetext,'*P*'),'/')"><xsl:value-of select="substring-before(substring-after(otherDataFreetext/longFreetext,'*P*'),'/')"/></xsl:when><xsl:otherwise><xsl:value-of select="substring-after(otherDataFreetext/longFreetext,'*P*')"/></xsl:otherwise></xsl:choose></xsl:attribute>
				</xsl:when>
				<xsl:when test="contains(otherDataFreetext/longFreetext,'*D*')">
					<xsl:attribute name="Amount"><xsl:choose><xsl:when test="contains(substring-after(otherDataFreetext/longFreetext,'*D*'),'/')"><xsl:value-of select="substring-before(substring-after(otherDataFreetext/longFreetext,'*D*'),'/')"/></xsl:when><xsl:otherwise><xsl:value-of select="substring-after(otherDataFreetext/longFreetext,'*D*')"/></xsl:otherwise></xsl:choose></xsl:attribute>
				</xsl:when>
				<xsl:when test="starts-with(otherDataFreetext/longFreetext,'PAX ') and contains(substring-after(otherDataFreetext/longFreetext,'PAX '),'A')">
					<xsl:attribute name="Amount"><xsl:value-of select="substring-before(substring-after(otherDataFreetext/longFreetext,'PAX '),'A')"/></xsl:attribute>
				</xsl:when>
				<xsl:when test="starts-with(otherDataFreetext/longFreetext,'PAX ') and contains(substring-after(otherDataFreetext/longFreetext,'PAX '),'P')">
					<xsl:attribute name="Percent"><xsl:value-of select="substring-before(substring-after(otherDataFreetext/longFreetext,'PAX '),'P')"/></xsl:attribute>
				</xsl:when>
				<xsl:when test="starts-with(otherDataFreetext/longFreetext,'INF ') and contains(substring-after(otherDataFreetext/longFreetext,'INF '),'A')">
					<xsl:attribute name="Amount"><xsl:value-of select="substring-before(substring-after(otherDataFreetext/longFreetext,'INF '),'A')"/></xsl:attribute>
				</xsl:when>
				<xsl:when test="starts-with(otherDataFreetext/longFreetext,'INF ') and contains(substring-after(otherDataFreetext/longFreetext,'INF '),'P')">
					<xsl:attribute name="Percent"><xsl:value-of select="substring-before(substring-after(otherDataFreetext/longFreetext,'INF '),'P')"/></xsl:attribute>
				</xsl:when>
				<xsl:when test="starts-with(otherDataFreetext/longFreetext,'PAX ')">
					<xsl:attribute name="Amount"><xsl:value-of select="substring-after(otherDataFreetext/longFreetext,'PAX ')"/></xsl:attribute>
				</xsl:when>
				...
				<xsl:otherwise>
					<xsl:attribute name="Percent"><xsl:value-of select="translate(otherDataFreetext/longFreetext,'ABCDEFGHIJKLMNOPQRSTUVWXYZ* ','')"/></xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
			<xsl:choose>
	...
	<xsl:template match="common_v28_0:AccountingRemark" mode="accounting">
		<AccountingLine>
			<xsl:value-of select="common_v28_0:RemarkData"/>
		</AccountingLine>
	</xsl:template>
	...
	<xsl:template match="itineraryInfo" mode="Taxi">
		...
		<Item>
			<xsl:attribute name="Status"><xsl:value-of select="relatedProduct/status"/></xsl:attribute>
			<xsl:attribute name="RPH"><xsl:value-of select="position()"/></xsl:attribute>
			<xsl:attribute name="ItinSeqNumber"><xsl:value-of select="elementManagementItinerary/reference[qualifier='ST']/number"/></xsl:attribute>
			<General>
				<xsl:attribute name="Start"><xsl:text>20</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,5,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,3,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/depDate,1,2)"/></xsl:attribute>
				<xsl:if test="travelProduct/product/arrDate != ''">
					<xsl:attribute name="End"><xsl:text>20</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,5,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,3,2)"/><xsl:text>-</xsl:text><xsl:value-of select="substring(travelProduct/product/arrDate,1,2)"/></xsl:attribute>
				...
				<TPA_Extensions>
					<xsl:attribute name="ActionCode"><xsl:value-of select="relatedProduct/status"/></xsl:attribute>
					<xsl:attribute name="NumberInParty"><xsl:value-of select="relatedProduct/quantity"/></xsl:attribute>
					<Vendor>
						<xsl:attribute name="Code"><xsl:value-of select="travelProduct/companyDetail/identification"/></xsl:attribute>
					</Vendor>
			...
			<Rail>
				<xsl:attribute name="Start">
					...
					<xsl:choose>
						...
						<xsl:otherwise><xsl:value-of select="substring(travelProduct/product/depTime,1,2)"/></xsl:otherwise>
					</xsl:choose>
				...
				<xsl:if test="travelProduct/product/arrDate != ''">
					<xsl:attribute name="End">
						...
						<xsl:choose>
							...
							<xsl:otherwise><xsl:value-of select="substring(travelProduct/product/arrTime,1,2)"/></xsl:otherwise>
						</xsl:choose>
	...
	<xsl:template match="air:AirPricingInfo" mode="totalTax">
		...
		<xsl:choose>
			<xsl:when test="../air:AirPricingInfo[$pos + 1]">
				<xsl:apply-templates select="../air:AirPricingInfo[$pos + 1]" mode="totalTax">
					<xsl:with-param name="sum">
						<xsl:value-of select="$tot + $sum"/>
					</xsl:with-param>
					<xsl:with-param name="pos">
