<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="Response">
		...
		<air:AirPriceReq>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<air:AirPricingModifiers>
						...
						<!--<xsl:if test="$PNR/air:AirReservation/air:AirPricingInfo[1]/air:BookingInfo/@CabinClass">
							<xsl:attribute name="CabinClass">
								<xsl:value-of select="$PNR/air:AirReservation/air:AirPricingInfo[1]/air:BookingInfo/@CabinClass"/>
							</xsl:attribute>
						</xsl:if>-->
			...
			<!-- AirPrice Command
			<xsl:if test="//StoredFare/BrandedFares/FareFamily">
			-->
...
xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ReturnRecord="true" AuthorizedBy="TRIPXML">
	...
	<xsl:template name="mod_air">
		...
		<xsl:for-each select="msxsl:node-set($groups)/elem/node()[1]">
			...
			<xsl:choose>
					...
					<universal:UniversalModifyCmd>
						...
						<universal:AirAdd>
							...
							<xsl:call-template name="ModStoredFareJoined" >
								...
								<xsl:with-param name="airprice" select="msxsl:node-set($airprice)[1]/node()[air:PassengerType/@Code = $ptc]" />
				...
				<xsl:when test="count(msxsl:node-set($airprice)[1]/node()[1]) = 1">
					<universal:UniversalModifyCmd>
						<xsl:attribute name="Key">
							<xsl:value-of select="position() + 1"/>
						</xsl:attribute>
						<universal:AirAdd>
							...
							<xsl:call-template name="ModStoredFareJoined" >
								...
								<xsl:with-param name="fares" select="$priceList[@FarePriceGroup=$g]" />
								<xsl:with-param name="ptc" select="$ptc" />
								<xsl:with-param name="airprice" select="msxsl:node-set($airprice)[1]/node()[1]" />
							</xsl:call-template>
						</universal:AirAdd>
					</universal:UniversalModifyCmd>
				</xsl:when>
	...
	<xsl:template match="air:AirPricingInfo" mode="brandFare">
		...
		<air:AirPricingCommand>
			<xsl:choose>
				...
				<xsl:when test="air:BookingInfo/@CabinClass">					
					<xsl:for-each select="../air:AirSegment">
						<xsl:variable name="pos" select="position()" />
						<air:AirSegmentPricingModifiers>
							<xsl:attribute name="AirSegmentRef">
	...
	<xsl:template match="air:AirPricingInfo" mode="brandFareStore">
		...
		<xsl:variable name="brandTier" select="air:FareInfo/air:Brand/@BrandTier" />
-->
		<xsl:variable name="brnds" >
			<xsl:call-template name="DistictList">
				<xsl:with-param name="list" select="air:FareInfo/air:Brand/@BrandTier"/>
			</xsl:call-template>
		</xsl:variable>
		...
		<air:AirPricingInfo>
			...
			<!-- [contains(msxsl:node-set($brnds)[1]/node()/text(),air:Brand/@BrandTier)]  based on example: 5SBG08 -->
	...
	<xsl:template name="ModStoredFareJoined">
		...
		<xsl:param name="ptc" select="$airprice[1]/air:PassengerType/@Code"/>

		<xsl:variable name="priceType" select="$fares[PassengerType/@Code = $ptc]/@FareType" />
		<xsl:variable name="pricing">
			<xsl:choose>
				<xsl:when test="contains($priceType,'Published')">
					<xsl:text>Guaranteed</xsl:text>
		...
		<xsl:variable name="crr" select="$fares[PassengerType/@Code = $ptc]" />

		<xsl:variable name="groups" >
			<xsl:choose>
				<xsl:when test="sum($pnr/air:AirReservation/air:AirPricingInfo/@AirPricingInfoGroup) > 1">
					<xsl:call-template name="DistictList">
						<xsl:with-param name="list" select="$pnr/air:AirReservation/air:AirPricingInfo/@AirPricingInfoGroup"/>
					</xsl:call-template>
				</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="DistictList">
						<xsl:with-param name="list" select="$airprice/@FarePriceGroup"/>
					</xsl:call-template>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<air:AirPricingTicketingModifiers>
			<xsl:choose>
				<xsl:when test="count(msxsl:node-set($groups)/elem/node()) > 1">

					<xsl:for-each select="$pnr/air:AirReservation/air:AirPricingInfo[air:PassengerType/@Code = $ptc]/@Key">
				...
				<xsl:otherwise>
					<xsl:for-each select="$fares">
						...
						<xsl:choose>
							<xsl:when test="$pnr/air:AirReservation/air:AirPricingInfo[air:PassengerType/@Code = $p]/@Key">
							...
							<xsl:otherwise>
								<air:AirPricingInfoRef>
									<xsl:attribute name="Key">
										<xsl:value-of select="$pnr/air:AirReservation/air:AirPricingInfo[1]/@Key"/>
									</xsl:attribute>
								</air:AirPricingInfoRef>
							</xsl:otherwise>
						</xsl:choose>
	...
	<xsl:template match="air:AirPricingInfo" mode="markup">
		...
		<xsl:if test ="$mu/@Amount and $storedFare/@FareType='Private'" >
			<!--<xsl:if test="format-number($mu/@Amount, '#0')!= 0.00">-->
