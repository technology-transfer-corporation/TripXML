...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
	...
	<xsl:variable name="SearchODByOneWay">
		<xsl:value-of select="OTA_AirLowFareSearchRS/OTA_AirLowFareSearchPlusRQ/@SearchODByOneWay"/>
	</xsl:variable>
	<xsl:variable name="EchoToken">
		<xsl:value-of select="OTA_AirLowFareSearchRS/OTA_AirLowFareSearchPlusRQ/@EchoToken"/>
	</xsl:variable>
	<xsl:template match="/">
	...
	<xsl:template match="OriginDestinationOption">
		<OriginDestinationOption>
			<xsl:attribute name="SectorSequence">
				<xsl:choose>
					<xsl:when test="$SearchODByOneWay='true'">
	...
	<xsl:template match="FlightSegment" mode="od">
		...
		<FlightSegment>
			...
			<xsl:choose>
				...
				<xsl:when test="contains(TPA_Extensions/Text,'OPERATED BY /') and OperatingAirline/@Code='**'">
					<OperatingAirline>
						<xsl:attribute name="Code"/>
