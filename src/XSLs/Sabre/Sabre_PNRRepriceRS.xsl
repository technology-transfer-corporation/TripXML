   ...
   -->
	...
	<xsl:template match="TravelItineraryReadRS">
		<xsl:choose>
			...
			<xsl:when test="OTA_AirPriceRS/Fault/detail/ApplicationResults/Error">
				<Errors>
					<Error>
						<xsl:attribute name="Type">Sabre</xsl:attribute>
						<xsl:value-of select="OTA_AirPriceRS/Fault/detail/ApplicationResults/Error/SystemSpecificResults/Message"/>
					</Error>
				</Errors>
			</xsl:when>
			<xsl:otherwise>
