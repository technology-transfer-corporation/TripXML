<?xml version="1.0"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl" xmlns:cs="urn:cs">
	<!-- 
================================================================== 
...
-->
	...
	<xsl:template match="DOC_ITM" mode="agencyComm">
		<xsl:if test="number(translate(CM_INF/CM_AMT, translate(CM_INF/CM_AMT,'.0123456789',''),''))=translate(CM_INF/CM_AMT, translate(CM_INF/CM_AMT,'.0123456789',''),'')">
			<AgencyCommission>
				...
				<xsl:variable name="paxBD">
					<xsl:choose>
						...
						<xsl:when test="string(number($paxName))='NaN' and (string-length($paxName) = 7 or string-length($paxName) = 9)">
							...
							<xsl:variable name="thisyr" select="number(substring(cs:datenow(),6))"/>
