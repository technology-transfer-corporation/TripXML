...
{
    public partial class wsCruiseModifyBooking
    {

        private readonly modMain _modMain;

        public wsCruiseModifyBooking(modMain modMain)
        {
            _modMain = modMain;
        }
        ...
        private string DecodeCruiseModifyBooking(string strResponse, string UserID, ref string strUUID)
        {
        ...
        private string FilterCruiseModifyBooking(string strRequest, string UserID)
        {
            XmlDocument oDoc = null;
            XmlElement oRoot = null;
            XmlNode oNode = null;
            ...
            try
            {
                ...
                ttCruiseBedConfiguration = (DataView)TripXMLMain.AppState.Get("ttCruiseBedConfiguration");
                ttCruisePaxTitle = (DataView)TripXMLMain.AppState.Get("ttCruisePaxTitle");
                ttwsCruiseInsurance = (DataView)TripXMLMain.AppState.Get("ttwsCruiseInsurance");
                ttwsCreditCards = (DataView)TripXMLMain.AppState.Get("ttwsCreditCards");
                ttCruiseOccupation = (DataView)TripXMLMain.AppState.Get("ttCruiseOccupation");
                ...
                foreach (XmlNode currentONode1 in oRoot.SelectNodes("ReservationInfo/GuestDetails/GuestDetail"))
                {
                    ...
                    foreach (XmlNode currentONodeChild in oNode.SelectNodes("ContactInfo"))
                    {
                        ...
                        {
                            if (oNodeChild.Attributes["EmergencyFlag"]?.Value == "true")
                            {
                        ...
                        if (modMain.GetCruiseFilterValue(ref ttCruiseProfiles, mstrVendorCode, "paxBirthDateSupported") != "true")
                        {
                            if (modMain.IsNothing(oNodeChild.Attributes["BirthDate"], "").ToString().Length > 0)
                            {
                ...
                foreach (XmlNode currentONode4 in oRoot.SelectNodes("ReservationInfo/PaymentRequests/PaymentRequest"))
                {
                    ...
                    else if (oNode.SelectSingleNode("PaymentCard") is not null)
                    {
                        ...
                        if (oNodeGrandChild is not null)
                        {
        ...
        private string ServiceRequest(string strRequest, ttServices ttServiceID)
        {
            ...
            try
            {
                ...
                strResponse = DecodeCruiseModifyBooking(strResponse, ttCredential.UserID, ref UUID);
            ...
            finally
            {
                ...
                if (modCore.Trace)
                    CoreLib.SendTrace(ttCredential.UserID, "wsCruiseModifyBooking", "============= OTA Response ============= ", strResponse, UUID);
            }
        ...
        #region  Web Methods 
        public wmCruiseCreateBookingOut.OTA_CruiseCreateBookingRS wmCruiseModifyBooking(wmCruiseCreateBookingIn.OTA_CruiseCreateBookingRQ OTA_CruiseCreateBookingRQ)
        {
            string xmlMessage = "";
        ...
        public string wmCruiseModifyBookingXml(string xmlRequest)
        {
