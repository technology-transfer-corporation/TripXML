...
Namespace wsTravelTalk
    ...
    <System.Web.Services.Protocols.SoapDocumentService(RoutingStyle:=System.Web.Services.Protocols.SoapServiceRoutingStyle.RequestElement),
        System.Web.Services.WebService(Namespace:="http://tripxml.downtowntravel.com/tripxml/wsLowFareSchedule",
        Name:="wsLowFareSchedule_v03",
        Description:="A TripXML Web Service to Process Low Fare Schedule Messages Request.")>
    Public Class wsLowFareSchedule_v03
...
#Region " Process Service Request All GDS "
        ...
        Private Function ServiceRequest(ByVal strRequest As String, ByVal ttServiceID As ttServices) As String
            ...
            Dim DoAmadeusWSSearches(99) As SearchSchAmadeusWS_v03
            ...
            Dim oRoot As XmlElement = Nothing
            Dim oNode As XmlNode = Nothing
            Dim oRootFT As XmlElement = Nothing

            Dim oNDoc As XmlDocument = Nothing
            Dim oNRoot As XmlElement = Nothing
            ...
            Try
                ...
                Dim testb As Boolean = False
                If oRoot.Attributes("PriorityDirectFlights") IsNot Nothing Then
                    testb = Boolean.Parse(oRoot.Attributes("PriorityDirectFlights").Value)
                End If

                If testb Then
                    ReqValue = 2
                    modCore.LFSchRequestCount = (oRoot.SelectNodes("FaringPreferences/FaringPreference").Count) * 2
                End If

                modCore.NonDirectFlights = testb

                sb.Append("<OTA_AirLowFareSearchScheduleRQ>").Append(oRoot.SelectSingleNode("POS").OuterXml)
                        ...
                        For k = 0 To ReqValue - 1
                            For Each oNode In oRoot.SelectNodes("FaringPreferences/FaringPreference")
                                If (ttCredential.Providers(i).PCC = oNode.Attributes("PseudoCityCode").Value) Then
                                    ...
                                    Dim strn As String = oNode.OuterXml
                                    tempTravelSum = TravelSum

                                    If Not oNode.Attributes("MaxResponses") Is Nothing Then
                                        tempTravelSum = tempTravelSum.Replace("<OTA_AirLowFareSearchScheduleRQ", "<OTA_AirLowFareSearchScheduleRQ MaxResponses=""" & oNode.Attributes("MaxResponses").Value & """")
                                    End If

                                    If Not oRoot.Attributes("PriorityDirectFlights") Is Nothing Then
                                        tempTravelSum = tempTravelSum.Replace("<OTA_AirLowFareSearchScheduleRQ", "<OTA_AirLowFareSearchScheduleRQ PriorityDirectFlights=""" & oRoot.Attributes("PriorityDirectFlights").Value & """")
                                    End If

                                    If Not oRoot.Attributes("OnlineAirlinesOnly") Is Nothing Then
                                        tempTravelSum = tempTravelSum.Replace("<OTA_AirLowFareSearchScheduleRQ", "<OTA_AirLowFareSearchScheduleRQ OnlineAirlinesOnly=""" & oRoot.Attributes("OnlineAirlinesOnly").Value & """")
                                    End If
                                    ...
                                    If intChars = -1 Then
                                        ...
                                        sb.Replace("<AirTravelerAvail>", repTravelSumString).Append("</TravelerInfoSummary></OTA_AirLowFareSearchScheduleRQ>")
                                    Else
                                        repTravelSumString = sb1.Append("</TravelPreferences><TravelerInfoSummary>").Append(uTravelSum).ToString()
                                        sb1.Remove(0, sb1.Length)
                                        sb.Replace("</TravelPreferences>", repTravelSumString).Append("</TravelerInfoSummary></OTA_AirLowFareSearchScheduleRQ>")

                                    End If

                                    Dim strt As String = sb.ToString()
                                    strRequest = sb.ToString()
                                    oNDoc = New XmlDocument
                                    oNDoc.LoadXml(sb.ToString())
                                    oNRoot = oNDoc.DocumentElement

                                    If testb Then
                                        If oNRoot.SelectSingleNode("TravelPreferences") IsNot Nothing Then
                                            Dim tt1 As String = oNRoot.OuterXml
                                            If k = 0 Then
                                                oNDoc.LoadXml("<FT><FlightTypePref FlightType='Nonstop'/></FT>")
                                            ElseIf k = 1 Then
                                                oNDoc.LoadXml("<FT><FlightTypePref FlightType='Connection'/></FT>")
                                            End If
                                            oRootFT = oNDoc.DocumentElement
                                            tt1 = oRootFT.SelectSingleNode("FlightTypePref").OuterXml
                                            oNRoot.SelectSingleNode("TravelPreferences").AppendChild(oRootFT.SelectSingleNode("FlightTypePref"))
                                            Dim tt As String = oNRoot.OuterXml
                                            strRequest = oNRoot.OuterXml
                                            oNRoot.SelectSingleNode("TravelPreferences/FlightTypePref").RemoveAll()
                                            tt = oNRoot.OuterXml
                                        Else
                                            If k = 0 Then
                                                oNDoc.LoadXml("<FT><TravelPreferences><FlightTypePref FlightType='Nonstop'/></TravelPreferences></FT>")
                                            ElseIf k = 1 Then
                                                oNDoc.LoadXml("<FT><TravelPreferences><FlightTypePref FlightType='Connection'/></TravelPreferences></FT>")
                                            End If

                                            oRootFT = oNDoc.DocumentElement
                                            oNRoot.AppendChild(oRootFT.SelectSingleNode("TravelPreferences"))
                                            'strSndRequest.Add(oNRoot.InnerXml);
                                            strRequest = oNRoot.OuterXml
                                            oNRoot.SelectSingleNode("TravelPreferences/FlightTypePref").RemoveAll()
                                        End If
                                    End If
                                        ...
                                        Case "amadeus"
                                            Try
                                                ...
                                                If ttProviderSystems.AmadeusWS = True Then
                                                    ...
                                                    waitTime = r.Next(200, 400)
                                                    Thread.Sleep(waitTime)

                                                    DoAmadeusWSSearches(j) = New SearchSchAmadeusWS_v03(.Providers(i).PCC, .UserID, .System, ttProviderSystems, oAmadeusWS)
                                                    DoAmadeusWSSearches(j).Request = lfstrRequest(j)
                                                    DoAmadeusWSSearches(j).ServiceID = CInt(ttServiceID).ToString()
                                            ...
                                            '        DoGalileoSearches(j).ServiceID =CInt(ttServiceID).ToString()
                                            ...
                                            '        DoSabreSearches(j).ServiceID =CInt(ttServiceID).ToString()
                                            ...
                                            '        DoWorldspanSearches(j).ServiceID =CInt(ttServiceID).ToString()
                                            ...
                                            '        DoPortalSearches(j).ServiceID =CInt(ttServiceID).ToString()
                                            ...
                                            '        DoPortalXMLSearches(j).ServiceID =CInt(ttServiceID).ToString()
            ...
            Finally
                ...
                If Trace Then CoreLib.SendTrace(ttCredential.UserID, "wsLowFareSchedule_v03", "============= OTA Response ============= ", strResponse, UUID)
            End Try
...
#Region " Web Methods "
        ...
        <WebMethod(Description:="Process Low Fare Schedule Messages Request.")>
        <System.Web.Services.Protocols.SoapHeader("tXML")>
        Public Function wmLowFareSchedule(ByVal OTA_AirLowFareSearchScheduleRQ As wmLowFareScheduleIn_v03.OTA_AirLowFareSearchScheduleRQ) As <XmlElementAttribute("OTA_AirLowFareSearchScheduleRS")> wmLowFareScheduleOut.OTA_AirLowFareSearchScheduleRS
            ...
            oSerializer = New XmlSerializer(GetType(wmLowFareScheduleIn_v03.OTA_AirLowFareSearchScheduleRQ))
            ...
            Catch ex As Exception
                CoreLib.SendTrace("", "wsLowFareSchedule_v03", "Error Deserialing OTA Response", ex.Message, String.Empty)
            End Try
...
#Region "Search Amadeus"
    'Public Class SearchAmadeus_v03
...
#Region "Search AmadeusWS"
    Public Class SearchSchAmadeusWS_v03
        Private Delegate Sub StartSearch_Delegate()
...
#Region "Search PortalXML"
    'Public Class SearchPortalXML_v03
    '    Private Delegate Sub StartSearch_Delegate()
